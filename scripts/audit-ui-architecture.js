...
const redundantRouteMediaHeightOverrides = routeStyles.flatMap((file) => {
  ...
  return [...source.matchAll(/@media[^{}]*\{[\s\S]*?\.pageFrame\s*\{[^}]*\bheight:\s*([^;]+);[^}]*\}/gu)]
    .filter((match) => match[1].trim() === baseHeight)
    .map(() => file)
})
record(
  'route_media_overrides_must_change_layout',
...
record(
  'all_online_wechat_verifiers_close_route_batches_before_connecting',
  onlineVerifiersWithLateBatchGuards.length === 0,
  onlineVerifiersWithLateBatchGuards.join(', ') || `scripts=${onlineVerifierBatchOrder.length}`,
)
...
record(
  ...
  fs.existsSync(path.join(root, 'scripts/promote-ui-region-comparison.js'))
    ...
    && /readBoundedFile/u.test(read('scripts/promote-ui-region-comparison.js'))
    && /writeBoundedFileImmutable/u.test(read('scripts/promote-ui-region-comparison.js'))
    && /exact mapping contract/u.test(read('scripts/promote-ui-region-comparison.js'))
...
record(
  ...
  /function validateManifestCacheIdentity\(manifestPath, manifest\)/u.test(reviewCachePromotion)
    ...
    && /cache manifest path does not match its commit and viewport identity/u.test(reviewCachePromotion)
    && /validateManifestCacheIdentity\(manifestPath, manifest\)/u.test(reviewIndexBuilder),
  'promotion and offline review must reject screenshots attributed to an unavailable commit or a mismatched cache directory',
)
record(
  ...
  /function capturePathMatches\(capture, cacheRoot\)/u.test(reviewCacheCapture)
    ...
    && /inspectCapture\(capture, viewport, manifestDirectory\)/u.test(reviewCachePromotion)
    && /inspectCapture\(capture, viewport, manifestDirectory\)/u.test(reviewIndexBuilder),
...
record(
  ...
  /packageMechanicsPass/u.test(packageAudit)
    ...
    && /WOW_ASSET_RUNTIME_ROOT must be an approved named HTTPS immutable \/releases\/<release-id> path/u.test(releaseDomainPolicy)
