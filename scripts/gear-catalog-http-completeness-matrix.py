#!/usr/bin/env python3
"""Run the all-spec Catalog-to-candidate HTTP completeness matrix."""
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--exact-registry-revision", required=True)
...
def main(argv=None) -> int:
    ...
    database_url = str(os.environ.get("WOW_DATABASE_URL") or "").strip()
    if (
        str(os.environ.get("WOW_DATABASE_RUNTIME") or "").strip()
        != "postgres_only"
        or not database_url
    ):
        raise SystemExit("PostgreSQL-only runtime is required")
    catalog = GearCatalogRevisionStore(
        lambda: connect_postgres(database_url)
    ).load_catalog(args.catalog_revision)
    ...
    report = run_catalog_http_completeness_matrix(
        ...
        exact_registry_revision=args.exact_registry_revision,
