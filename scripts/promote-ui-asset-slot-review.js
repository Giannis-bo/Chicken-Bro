...
function combineDetails(details) {
  ...
  if (details.some((detail) => detail.schemaVersion !== 'wechat-runtime-asset-slot-review-v3')) throw new Error('unsupported asset-slot detail schema')
  ...
  if (routes.some((route) => route.status !== 'pass' || route.elementCount < 1 || route.assetElementCount < 1 || route.slotCount < 1 || route.missingAssetElements !== 0 || route.missingPromotionStatusElements !== 0 || !Number.isInteger(route.materialOwnerElementCount) || route.materialOwnerElementCount < 0 || route.invalidMaterialOwnerElements !== 0 || route.materialOwnerCounts?.asset + route.materialOwnerCounts?.css !== route.materialOwnerElementCount || route.failures.length !== 0 || route.semanticMappings.length !== route.slotCount)) throw new Error('only complete passing non-empty asset-slot reviews may be promoted')
  const byRoute = new Map(routes.map((route) => [route.route, route]))
  return {
    schemaVersion: 'wechat-runtime-asset-slot-evidence-v3',
    commit: first.commit,
