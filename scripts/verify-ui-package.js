...
function verifyRuntimeMediaReleaseRoot(value) {
  ...
  const immutableRoot = value.replace(/\/+$/u, '')
  const manifestUrl = `${immutableRoot}/release-manifest.json`
  ...
  const valid = manifest?.schemaVersion === 1
    ...
    && manifest?.immutableRoot === immutableRoot
...
function build(name, assetRuntimeRoot = '') {
  ...
  return {
    ...
    runtimeMediaRootReferenceCount: runtimeMediaRoot ? countStringLiteral(packageText, runtimeMediaRoot) : 0,
...
try {
  ...
  if (runtimeMediaRoot && (!local.runtimeMediaRootReferenceCount || !remote.runtimeMediaRootReferenceCount)) {
    failures.push('weapp package does not contain the configured runtime media root')
  }
