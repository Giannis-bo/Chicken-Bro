...
**Context:** The compatibility rollout first keeps the existing endpoint response shape while moving it to the lightweight `stat_snapshot_v1` execution mode and enforcing a global concurrency limit. New clients will use `/api/websim/gear/stat-snapshots` backed by the PostgreSQL snapshot store and systemd worker. Observe client-version and endpoint-call telemetry for at least two client release cycles, define an explicit retirement threshold and rollback plan, then deprecate and eventually return `410 Gone` for the legacy route. New frontend code must not call the legacy endpoint.
