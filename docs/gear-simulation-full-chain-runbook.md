...
- 当前 `cache.websim_items`、gear source/variant/mod option 和 community template 表继续是 mutable staging。正式 release-scoped 表尚未创建；当前仍为 `compatibility-pg-live-v1`、`formalActiveManifest=false`，不能把计划写成已实现能力。
