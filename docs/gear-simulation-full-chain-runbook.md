...
- Slice 3C 是 active page cutover：必须把 initial/candidate/variant/enhancement/community/saved/reset 七条确认流统一到同一 Intent/Resolve 路径，最终属性、Tier/set、legality、constraints 与 readiness 全部来自匹配 snapshot。真实微信 initial verified、edit pending→resolved、safe problem/read-only 以及候选 backend 全门禁通过前不得合入。
