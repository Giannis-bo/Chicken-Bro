...
- 2026-07-11 Phase 5A live acceptance：PR #79 squash 合入 `8a75fd5`。migration 0015、revision-complete signature、immutable verified snapshot、single-flight、limits/cooldown、fenced lease/reclaim 与 bounded health repository 已从 clean main 部署；真实应用连接 smoke 完成 duplicate reuse、claim、heartbeat、stale-token reject、publish/reload 并回滚至 0/0/0，active generation 9 不变。当前 [Phase 5 Async Stat Snapshot Plan](plans/2026-07-11-equipment-simulator-phase5-async-stat-snapshot-plan.md) 进入 5B：只允许 canonical async API、一个 SimC child worker/service、bounded operations 与 legacy telemetry；前端 cutover 等待 5C，Catalyst 保持 disabled/TODO。
- 2026-07-11 Phase 5B candidate acceptance：exact implementation `c44e872` 通过 canonical API、稳定单 worker、separate-connection heartbeat、重解析/signature drift、verified JSON-only publication、bounded health/admin 与 legacy telemetry 门禁。候选真实双并发复用 job 9，202 均低于 500ms，三次 poll 后 200 verified；worker 停止 30 秒后 cached hit 仍 200、新 race signature 为 `GEAR_STAT_WORKER_UNAVAILABLE` 503 且不入队。真实事务完成 expired reclaim 与 stale token reject 后回滚。cache-hit localhost p95 `93.1ms`；worker one task、约 27 MiB current / 117 MiB peak、`NRestarts=0`，backend 单 task/零重启。最终 3 immutable snapshots / 3 completed jobs / 1 stable worker row，queue 0；40/40 public observed-only、legacy stats、Catalyst fail-closed、generation 9、runtime parity、timer/backflow/log 通过。5B 合入前仍需 exact-head PR/CI；5C 前端切换未开始。
- 2026-07-12 Phase 5C/5D closure：PR #81 完成 structured async frontend consumer，clean-main smoke 发现的 slim-payload 模板丢失由 TDD 修正 PR #82 合入 `d02d885`。真实微信 clean main 无手工 reload 保持 observed template，并完成 pending-to-verified；changed input 期间旧 snapshot 只读 stale，随后由新 signature 替换。40-spec final matrix 为 32 verified immutable snapshots 与 8 explicit non-ready（6 `GEAR_STAT_SIMC_FAILED`、2 `GEAR_STAT_JSON_INVALID`），非 ready 精确对应当前 SimC unsupported healer、DH 双持装备缺口或空 `buffed_stats`，snapshot fabrication=0。cache-hit p95 `84.8ms`，worker/backend 单 task、零重启、active queue=0；PG-only、11-file runtime parity、public observed-only/baseline-empty、generation 9、Catalyst fail-closed、自然 timer 成功完成和空 error journal 通过。Phase 0-5 已归档；Phase 6 必须另开 Harness contract，当前不得启用 Catalyst 或前端合成。
