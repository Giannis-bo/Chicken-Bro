...
后台通过 `game=wow|poe2` 切换，省略时默认魔兽世界；对话按持久化会话游戏筛选，SimC 与 PoB 分别聚合。用户按该游戏实际提问去重；累计用户截止所选结束日，新增按该游戏首次提问日期，活跃仅计算期间提问用户。两款游戏都提问过的账号各计一次；仅登录、空会话、构筑导入或计算不计入对话用户。POE2 构筑导入按创建日期统计（含软删除历史），任务复用不重复计数。

统计使用北京时间区间与只读快照事务：活跃用户按当前游戏提问去重；成功率排除进行中与取消，缺有效指标或来源的 SimC 成功状态计作异常失败；POE2 成功要求数值指标、正生命或能量护盾以及绑定源构筑的来源信息，异常结果单列。排除已知验收模拟身份，不推算页面访问或费用。
