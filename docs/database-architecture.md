...
Migration `0019_gear_evidence_registry` adds the dormant Gear Evidence Registry. `cache.websim_gear_evidence_artifacts`, `cache.websim_gear_evidence_observations`, `cache.websim_gear_canonical_facts`, and `cache.websim_gear_evidence_invalidations` are append-only to `wow_app`; correction is represented by a new immutable record or invalidation, never by updating or deleting historical evidence. Artifact and Observation writes are idempotent by their public canonical identities, and Fact versions are distinct by `(fact_key, fact_value_hash, provenance_hash)`. `server/gear_evidence_store.py` is the sole SQL owner for those records and revalidates Artifact, Observation, and Fact inputs through the public constructors in `server/gear_evidence_registry.py`.

`ops.websim_gear_evidence_gaps` is an operational queue only. It stores a semantic `fact_key`, bounded root cause, structured missing requirement, retry state, and lease/CAS fence; it never stores a Fact value and cannot update Canonical Fact rows. `server/gear_evidence_gap_store.py` is the sole queue SQL owner and exposes only idempotent enqueue, fenced claim/finish, and bounded health aggregation. Migration `0021_gear_evidence_manual_pending` adds the terminal `manual_pending` state for `unverified_observed_capacity`: raw observed socket occupancy stays visible as a bounded missing requirement, but no generic stat proof or unsupported worker can turn it into a canonical capacity. This persistence slice does not construct Gear Releases, move Manifest pointers, change Resolver behavior, activate a release, or alter Taro.
