...
当前产品：Web Chat、云端 SimC、QQ 网站登录。先按任务选入口，历史资料仅在需要证据时展开。

| 要做的事 | 入口与职责 |
| --- | --- |
| 使用产品 | [使用指南](user-guide.md)：登录、对话、历史与模拟 |
| 了解当前方向 | [路线图](roadmap.md)：已完成、正在推进、下一步及待决策 |
| 确认任务范围 | [计划白名单](plans/README.md)：持续工作流、当前任务和历史索引 |
...
- 历史计划、发布包和恢复证明保留原记录，通过[历史索引](plans/README-history-20260911.md)按需查阅；归档不改变失败、预算或未验证状态，也不恢复旧 Mini/微信发布与迁移待办。
