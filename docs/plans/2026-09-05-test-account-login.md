...
复用现有 ChatModel，无 API、服务端或身份变更。验证覆盖等待/流式回复/完成/失败/断线、计时清理与下一请求重置，以及已有 Enter、Shift+Enter、中文输入法和草稿保留回归；完成类型检查、lint、前端与控制面测试、测试 H5 构建，并检查桌面与窄屏。只发布隔离测试 Web，保留旧 release 供回滚；用户体验接受、main 整合仍单独处理。
