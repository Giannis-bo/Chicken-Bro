...
状态：`正在推进（Phase 2 已归档；Phase 3–5 未授权）`
分类：`Strict（分阶段实施）`
确认日期：2026-08-02

本设计扩展[炸鸡队长统一 ChatBot 与受控分析架构](2026-07-24-chickenbro-chatbot-design.md)和[来源驱动 Agent 实施计划](2026-08-01-chickenbro-source-agent-implementation.md)。它定义长期能力演化边界，但不把现有来源 Agent 描述成已经具备 Toolsmith 或自动晋级能力。Phase 1 与 Phase 2 Tool Registry 已归档；Phase 3–5 仍必须分别规划、审阅、验证和发布，不能从本次授权推导。

已归档证据：[Phase 1：Trace、Outcome 与 Eval](../../artifacts/releases/2026-08-02-chickenbro-observability-phase1/evidence.json) · [Phase 2：Tool Registry](../../artifacts/releases/2026-08-02-chickenbro-tool-registry-phase2/evidence.json)
...
已归档的 Phase 2 证据冻结了专项边界：初始 Registry 只包含 Raider.IO 与 Warcraft Logs；个人模板和 SimC 继续作为 context evidence；Registry 只驱动发现，执行仍绑定仓库内获准 adapter。
...
阶段不得合并成一次性切换。Phase 1 已归档，Phase 2 已完成本地实现并进入独立候选验证；Phase 3–5 没有执行授权，必须使用 Phase 2 的新鲜归档证据重新决策。
