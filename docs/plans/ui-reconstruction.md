...
| 核心交互 | `core-interaction-contract.json` 已为 14 路由各登记一个源码 owner、稳定选择器、前置条件、动作与预期；最新真实微信长批次首次得到 10 条直接通过，builds home、news list、chickenbro 因批次后段连接延迟失败后在同一 DevTools 实例定向复跑通过，合计 13 路由核心交互通过。`talent` 合同显式登记 `stale/error/blocked/empty` 前置不可用状态，验证器仅在必需 tab 缺失且实际 routeState 命中合同枚举时记为 `UNAVAILABLE`，其他超时和动作失败仍为 `FAIL`；本轮结果为 `UNAVAILABLE/stale`、coverage true、failures 0。装备选择保持本地草稿先落地、选择面板先关闭，再异步校验；完整不可变证据记录仍为 `UNVERIFIED` | 将 13 条通过结果与 `talent` 前置不可用诊断写入逐路由不可变证据；生产请求域名批准后补跑 `talent`，不 launch/重载 DevTools |
