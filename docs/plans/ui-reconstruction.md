...
| 包体与远端资源 | 默认本地素材构建约 5.4 MB；显式 HTTPS 素材根构建约 1.9 MB 且不复制本地素材，但共享 JS/CSS 仍各约 600 KB。生产资产上传、不可变 URL 与微信 request/download 合法域名审批尚未形成可验证记录 | COS/CDN、不可变 URL、域名审批、代码分割和包体预算在同一资源交付工作包内关闭 |
