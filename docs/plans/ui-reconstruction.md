...
素材状态证据约束：所有发布 `data-asset-id` 的五类渲染 owner 必须同时从统一 manifest 发布 `data-promotion-status=production_promoted/candidate_pending_review/missing`。素材槽 detail v2 记录逐路由生产、候选和缺失计数；缺少合法状态的可见素材不能晋级。该证据只区分生产与候选，不会把“已接线”冒充“已完成真机与人工复核”。
