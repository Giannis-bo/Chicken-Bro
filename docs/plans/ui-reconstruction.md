...
| 包体与远端资源 | 2026-07-18 已将 135 个 runtime 素材（66 个 SVG、69 个 raster 2x，共 3,230,188 bytes）上传到不可变根 `https://static.chickenbro.cloud/wow-assets/releases/2026-07-18-ui-v2`；COS 与 CDN 逐文件复核 135/135，HTTP、字节数和 SHA-256 均通过。CDN 证书 `ZIfriY0E` 有效至 2026-10-16，样例请求命中 CDN Cache Hit，微信后台已由用户确认加入 `downloadFile` 合法域名。仓库现提供 `prepare:ui-cdn`、`publish:ui-cdn`、`verify:ui-cdn` 三段式版本化发布通路，发布清单记录逐文件 byte/SHA-256，上传后从 CDN 全量回读；凭据只由本机 `coscli` 管理。真实资源根下包体机械预算已通过，但 API 仅完成 DNS 与证书签发，服务器 HTTPS 反代仍因实例权限阻塞，因此整体仍为 `releaseReady: false` | 获得实例部署权限后完成 API HTTPS smoke，再以同一不可变资源根运行 `npm run verify:ui-package:release`；未来素材变化必须换新 `release-id`，不得覆盖已发布目录 |
