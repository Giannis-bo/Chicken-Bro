...
| 素材生产链 | 14 个路由素材合同均已登记：`news_home` 1 个已进入微信生产状态，9 个仅允许部分复用，2 个未晋级，2 个尚无路由素材晋级。05、06、任务列表、任务详情已增加运行子槽到概念合同槽/共享非事实槽的显式映射；真实微信扫描 140 个可见素材元素、49 个唯一运行槽，未登记槽和 `asset-missing=true` 均为 0。该结果只关闭槽位归属和缺失状态，不证明材质语义、清晰度或 production 晋级；槽位级仍有 15 个绑定生成待完成、7 个生成待隔离审查，以及多个复用/真机视觉验证待晋级槽位 | 素材进入统一 manifest，完成裁切、语义、清晰度和真机复核后晋级；COS/CDN 迁移作为独立工作包执行 |
