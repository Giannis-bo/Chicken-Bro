...
| 包体与远端资源 | 2026-07-18 已将 135 个 runtime 素材（66 个 SVG、69 个 raster 2x，共 3,230,188 bytes）上传到不可变根 `https://static.chickenbro.cloud/wow-assets/releases/2026-07-18-ui-v2`；COS 与 CDN 逐文件复核 135/135，HTTP、字节数和 SHA-256 均通过。CDN 证书 `ZIfriY0E` 有效至 2026-10-16，样例请求命中 CDN Cache Hit，微信后台已由用户确认加入 `downloadFile` 合法域名。仓库现提供 `prepare:ui-cdn`、`publish:ui-cdn`、`verify:ui-cdn` 三段式版本化发布通路，发布清单记录逐文件 byte/SHA-256，上传后从 CDN 全量回读；凭据只由本机 `coscli` 管理。真实微信端复核证明 SVG 与 CSS mask 整体不可靠，微信包现内置由 66 个注册 SVG 确定性生成的 96×96 PNG 派生物并由原生 `<image>` 渲染，3 MB 级美术 raster 仍走 CDN；源 SVG 继续参与发布与审计。以真实 CDN 根、`https://api.chickenbro.cloud` 和已确认微信 request 审批运行 `npm run verify:ui-package:release` 必须保持 `releaseReady: true` 且总包低于 2 MB。开发者工具日志曾确认旧的一次性构建会先清空 `dist/weapp`，在 `app.json` 重建前造成全路由 `updateAppCode` 与 `summer-compiler miss`，可直接触发 appLaunch 超时和白屏；当前 `build:weapp` 先在项目树外完整构建和校验，只晋级内容变化的文件，保持 `app.json` 路径存续并最后更新启动入口，失败构建不再触碰活动包 | 未来素材变化必须换新 `release-id`，上传后全量回读，再运行相同生产门禁；不得覆盖已发布目录；微信包只内置轻量系统图标 PNG，不得重新内置大体积美术 raster；一次性构建必须继续通过 staged promotion，不得恢复原地清空输出目录 |
| 微信 API 请求域名 | 2026-07-18 已创建 `api.chickenbro.cloud` A 记录并指向上海轻量云实例 `124.223.51.33`，腾讯云免费证书 `ZIppfWcD` 已部署到 Nginx，有效期至 2026-10-16。独立 server block 仅对该域名把 HTTP 301 到 HTTPS，并在 443 反代 `127.0.0.1:8787`；原有 HTTP IP/default server 保留为回滚入口。绕过本机代理并固定直连 `124.223.51.33` 的严格 TLS smoke 返回 HTTP 200、`ssl_verify_result=0`，证书 CN/SAN 均为 `api.chickenbro.cloud`；HTTP 返回 301 到同路径 HTTPS。`/api/data/health` 的 backend 组件为 `verified`，整体数据健康仍如实为 `partial`。服务器配置备份位于 `/var/backups/wow-nginx/` | 证书到期前续签并替换同路径证书，执行 `nginx -t` 后 reload；持续以严格 TLS health smoke 和生产包体门禁阻断回归 |
