...
| 真实微信视口边界 | `route-geometry-contract.json` 与 `npm run verify:ui-route-geometry` 已把 14 路由共享 shell、顶层 RouteRegion 和原生 Button 纳入 1px 容差门禁；现有 390×844 DPR3 微信实例分批复跑 14/14 通过，顶层区域最大右边界 383.14px，05/06/12/Profile 终端底部分别为 843/842/843.77/843.95px。只有 `data-visible-slot=false` 的 06 内部滚动卡片和具名 `simc-specialization-option` 横向滚动内容可越出视口，失败明细硬限制为前 10 条，防止异常输出拖死会话 | CI 持续运行静态合同门禁；涉及几何的变更在既有 Automator endpoint 上按路由小批复跑，合法滚动例外必须具名登记，不能用全局 overflow 豁免 |
