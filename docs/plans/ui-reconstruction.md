...
| 系统问题 | 当前事实 | 关闭条件 |
| --- | --- | --- |
| 全量视觉闭环 | `runtime-review-status.json` 已逐路由绑定 path、canonical target、合同根与当前状态；真实微信已完成 14 路由结构/几何扫描，并在全量与定向复跑合并后观察到 13 路由核心交互通过，`talent` 因请求域名前置不可用明确返回 `routeState=stale`、tab 数量为 0。截图链路已分为仓库外 commit/viewport 缓存和显式不可变晋级：`9bb2f3c69a45` 的 390×844 DPR3 实例已为 05、06、任务列表和任务详情晋级 4 份内容寻址 artifact 与 receipt。四路由的 26 个语义区域已通过结构化 target/runtime 映射试跑，位置最大残差分别为 2.44/6.91/0.38/0.04px，尺寸最大残差为 3.11/3.19/0.92/0.79px，均在目标图手工测量允许的 8px 位置和 4px 尺寸容差内；比较器不读取图片像素，结果仍需绑定到提交后的不可变证据。四路由还缺素材语义和人工确认，其余路由还缺不可变 runtime artifact，因此 14/14 继续保持 `UNVERIFIED` | 每路由按 `runtime-review-contract.json` 提交完整结论，并由人工确认后单独晋级 |
