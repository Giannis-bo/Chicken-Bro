...
| 素材生产链 | 14 个路由素材合同均已登记：`news_home` 1 个已进入微信生产状态，9 个仅允许部分复用，2 个未晋级，2 个尚无路由素材晋级。14/14 路由现均有运行子槽到概念合同槽/共享非事实槽的显式映射；`16e4da777937` 的 390×844 DPR3 微信实例分批扫描 506 个可见素材元素、路由内累计 145 个唯一槽，未登记槽、`undefined` 槽和 `asset-missing=true` 均为 0，并已合并为 SHA-256 `e75fb966…fc16` 的内容寻址全量证据。跨路由同名运行槽必须在 `sharedSlots` 登记且统一映射为唯一 canonical shared role；反向的 assetId→slot 审计对 154 个字面量绑定实行 deny-by-default，只有带理由登记的系统 utility/source badge 家族、同一鸡哥身份和同一证据折叠箭头可以跨语义槽，未登记 raster/材质复用直接失败。news detail 证据标题曾误用 `asset_slot.news-metric-glyphs`，已改归 `asset_slot.news-detail-evidence-header`，真实微信定向扫描 17 元素 / 8 槽、missing 0、旧槽 0。该结果关闭槽位归属和明确语义碰撞，不证明材质清晰度或 production 晋级；槽位级仍有 15 个绑定生成待完成、7 个生成待隔离审查，以及多个复用/真机视觉验证待晋级槽位 | 素材进入统一 manifest，完成裁切、语义、清晰度和真机复核后晋级；COS/CDN 迁移作为独立工作包执行 |
...
| 真实微信视口与安全区 | `route-geometry-contract.json` 与 `npm run verify:ui-route-geometry` 已把 14 路由共享 shell、顶层 RouteRegion、原生 Button 横纵边界和设备 safe area 纳入 1px 容差门禁；非 unavailable 路由若没有语义区域会以 `missing-route-regions` 失败。真实微信为 390×844 DPR3、safeAreaBottom 810、safeBottomInset 34；修正后的全量复跑 14/14 通过且零路由为零区域。固定 dock 控件必须直接结束在安全底边之上；Chickenbro composer 原先按钮 bottom 831，根因是 pushed plain dock 未继承 safe padding，现由共享 AppShell owner 统一加底部安全区，两个按钮 bottom 均为 797。SimC 与 12 的正文按钮可分别到 844 / 830.23，但仅因 shellBody 确实可滚且 paddingBottom 58≥34 才允许完整滚入安全区；不可滚或 padding 不足会失败。06 内部滚动卡片和具名横向内容仍使用精确角色例外，失败明细硬限制前 10 条 | CI 持续运行静态合同门禁；涉及几何的变更在既有 Automator endpoint 上按路由小批复跑，固定 dock 与可滚正文使用不同安全区规则，不能用全局 overflow 豁免 |
