...
| 系统问题 | 当前事实 | 关闭条件 |
| --- | --- | --- |
| 全量视觉闭环 | `runtime-review-status.json` 已逐路由绑定 path、canonical target、合同根与当前状态；真实微信已完成 14 路由结构/几何扫描，并在全量与定向复跑合并后观察到 13 路由核心交互通过，`talent` 因请求域名前置不可用明确返回 `routeState=stale`、tab 数量为 0。截图链路已分为仓库外 commit/viewport 缓存和显式不可变晋级：`9bb2f3c69a45` 的 390×844 DPR3 实例已为 05、06、任务列表和任务详情晋级 4 份内容寻址 artifact 与 receipt。`93f0cbd6d065` 上重新采集的四路由 26 区域 target/runtime 比较已作为 SHA-256 `859ba24b…39ff` 的内容寻址 JSON 晋级并登记；位置最大残差分别为 2.44/6.91/0.38/0.04px，尺寸最大残差为 3.11/3.19/0.92/0.79px，均在目标图手工测量允许的 8px 位置和 4px 尺寸容差内。比较器不读取图片像素，四路由仍缺素材语义和人工确认，其余路由还缺不可变 runtime artifact，因此 14/14 继续保持 `UNVERIFIED` | 每路由按 `runtime-review-contract.json` 提交完整结论，并由人工确认后单独晋级 |
| 素材生产链 | 14 个路由素材合同均已登记：`news_home` 1 个已进入微信生产状态，9 个仅允许部分复用，2 个未晋级，2 个尚无路由素材晋级。14/14 路由现均有运行子槽到概念合同槽/共享非事实槽的显式映射；真实微信分批扫描 506 个可见素材元素、路由内累计 145 个唯一槽，未登记槽、`undefined` 槽和 `asset-missing=true` 均为 0。`AppShell` 已停止把可选槽编译为 `undefined`，新闻 feed 旧占位槽已并入统一 `asset_slot.news-feed-fallback`。该结果只关闭槽位归属和缺失状态，不证明材质语义、清晰度或 production 晋级；槽位级仍有 15 个绑定生成待完成、7 个生成待隔离审查，以及多个复用/真机视觉验证待晋级槽位 | 素材进入统一 manifest，完成裁切、语义、清晰度和真机复核后晋级；COS/CDN 迁移作为独立工作包执行 |
...
| 真实微信视口边界 | `route-geometry-contract.json` 与 `npm run verify:ui-route-geometry` 已把 14 路由共享 shell、顶层 RouteRegion 和原生 Button 纳入 1px 容差门禁；74 个 `RouteRegion` 现由类型系统强制携带稳定 `data-region`，运行门禁拒绝任何退化为位置编号的匿名区域。现有 390×844 DPR3 微信实例分批复跑 14/14 通过，顶层区域最大右边界 383.14px，05/06/12/Profile 终端底部分别为 843/842/843.77/843.95px；补名后真实抽检 05、06、任务列表、任务详情共 26/26 区域均恢复出语义 ID且零越界。只有 `data-visible-slot=false` 的 06 内部滚动卡片和具名 `simc-specialization-option` 横向滚动内容可越出视口，失败明细硬限制为前 10 条，防止异常输出拖死会话 | CI 持续运行静态合同门禁；涉及几何的变更在既有 Automator endpoint 上按路由小批复跑，合法滚动例外必须具名登记，不能用全局 overflow 豁免 |
