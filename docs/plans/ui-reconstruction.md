...
| 包体与远端资源 | 2026-07-18 已将 135 个 runtime 素材（66 个 SVG、69 个 raster 2x，共 3,230,188 bytes）上传到不可变根 `https://static.chickenbro.cloud/wow-assets/releases/2026-07-18-ui-v2`；COS 与 CDN 逐文件复核 135/135，HTTP、字节数和 SHA-256 均通过。CDN 证书 `ZIfriY0E` 有效至 2026-10-16，样例请求命中 CDN Cache Hit，微信后台已由用户确认加入 `downloadFile` 合法域名。仓库现提供 `prepare:ui-cdn`、`publish:ui-cdn`、`verify:ui-cdn` 三段式版本化发布通路，发布清单记录逐文件 byte/SHA-256，上传后从 CDN 全量回读；凭据只由本机 `coscli` 管理。以真实 CDN 根、`https://api.chickenbro.cloud` 和已确认微信 request 审批运行 `npm run verify:ui-package:release`，结果为 `status: pass`、`releaseReady: true`、`releaseBlockers: []`；远端包 1,548,456 bytes / 77 文件 / 0 本地素材文件 | 未来素材变化必须换新 `release-id`，上传后全量回读，再运行相同生产门禁；不得覆盖已发布目录 |
