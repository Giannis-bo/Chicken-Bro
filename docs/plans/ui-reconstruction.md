...
| 包体与远端资源 | 2026-07-18 已将发布所需的 135 个 runtime 素材（66 个 SVG 与 69 个 raster 2x 文件，共 3,230,188 bytes）上传至不可变 HTTPS 根 `https://static.chickenbro.cloud/wow-assets/releases/2026-07-18-ui-v2`；COS 源站与 CDN HTTPS 分别逐文件复核 135/135，HTTP 200、byte/SHA-256 mismatch 均为 0。CDN 已绑定 `static.chickenbro.cloud` 的腾讯云托管证书 `ZIfriY0E`，证书 SAN 与域名一致、链验证通过，有效期至 2026-10-16；样例文件连续请求均命中 CDN Cache Hit，且与仓库 SHA-256 一致。微信公众平台已由用户确认将 `https://static.chickenbro.cloud` 加入 `downloadFile` 合法域名。`npm run verify:ui-asset-integrity` 当前复跑覆盖 6 个 raster collection、69 个 asset ID、138 个 1x/2x runtime 文件，共 3,807,528 bytes，failure 0。以真实 `WOW_ASSET_RUNTIME_ROOT` 复跑 `npm run verify:ui-package`：本地素材包 4,807,273 bytes / 214 个文件 / 137 个素材文件；远端素材包 1,541,686 bytes / 77 个文件 / 0 个素材文件，`common.js` 349,226 bytes、`common.wxss` 146,559 bytes，机械预算通过。当前报告仍为 `status: partial`、`packageMechanicsPass: true`、`releaseReady: false`；微信后台虽已由用户确认存在两个 API `request` 条目，但候选后端域名公网 DNS/HTTPS 验证失败，生产 `WOW_BACKEND_API_BASE_URL` 仍不可配置。不得用后台条目存在替代域名可达性证据，也不得将 CDN 迁移完成误报为小程序整体可发布 | 为选定后端域名完成 DNS、TLS 与 HTTPS API 反代，严格验证 `/api/data/health` 后再设置 `WOW_BACKEND_API_BASE_URL` 与 `WOW_WECHAT_REQUEST_DOMAIN_APPROVED=yes`，以相同不可变资源根运行 `npm run verify:ui-package:release` 得到 `releaseReady: true`。路径闭包、字节哈希和机械预算由隔离门禁持续阻断回归 |
| 微信 API 请求域名 | `npm run audit:taro-domain` 为 report-only：开发 origin 与 H5 proxy 均为显式 HTTP IP，认证请求在非 HTTPS 下 fail-closed，仓库未提交域名校验绕过。2026-07-18 已创建 `api.chickenbro.cloud` A 记录并指向现有上海轻量云实例 `124.223.51.33`，免费证书 `ZIppfWcD` 已签发，有效期至 2026-10-16；当前真实后端 `http://124.223.51.33/api/data/health` 返回 200。证书尚未部署到服务器：当前腾讯云子账号可管理域名和证书，但 OrcaTerm/TAT 无该实例登录权限，轻量云自动部署又不支持该实例镜像。因此 HTTPS 反代与严格 TLS smoke 未完成，`WOW_BACKEND_API_BASE_URL` 仍不得配置，`productionReady` 保持 false | 由实例所有者或具备 Lighthouse/TAT 权限的账号部署 `ZIppfWcD`，配置 `api.chickenbro.cloud` 的 443 反向代理并确认 `/api/data/health` 严格证书校验返回 200；随后记录对应微信 `request` 审批并复跑发布门禁 |
