...
- 2026-09-06 用户反馈元素萨满报告大量显示“待收录”。原名称表以增强萨满为主，漏掉元素技能、召唤物及饰品/套装增益。本轮按运行引擎 f50a2121 的法术/NPC 编号核实国服数据库名称，补齐真实 Giannis 报告的 10 项技能、27 项增益；保留来源和不同属性效果，不修改报告数值。用原始报告固定样本验证 14 项技能和 43 项增益完整覆盖、显示数值不变；未知标识仍不得猜译。
...
- 标签切换修复版 `445f98f8` 已部署测试站：79 项网页/对话测试、62 项控制面测试、类型/lint/网页构建通过。实际新会话等待时切至模拟页，后台生成六段完整回答，返回及刷新后均可见；模拟草稿保留，桌面及 390px 无新增溢出。后端 107 个文件、生产服务及 SimC 身份保持；用户验收仍待完成。

- 2026-09-06 用户指定两条示例文案为“Raider.IO 链接格式参考：”和“WCL 链接参考格式参考：”，每行直接接链接，删除 Giannis－白银之手示例标题；桌面同行及 390px 换行检查通过。随后要求页头直接使用 SIMC，标题及版本卡片统一为 SIMC，移除云端副标题。测试站 `416c8f59` 实际显示 SIMC 1210-01、游戏 12.1.0.69299；22 项工作台测试、类型/lint/网页构建通过。

- 元素报告译名修复 `881ad495` 已发布测试站；206 前端、62 控制面、类型/lint、双端构建通过。实际 WCL 任务 `7e2c4903-6de4-4049-a0f6-ea7f36118af3` 的 14 技能、43 增益均显示中文名称，待收录为 0；原报告 SHA256 `659d00a8fe15a56d443ffc01101eed2e99de7ea2dd3f4776a5ab3f150cca2b45` 与固定样本一致，每秒伤害 227723.910084102 不变。桌面/390px 检查通过。未知其他报告标识仍如实降级；不宣称所有职业全量名称已覆盖。
