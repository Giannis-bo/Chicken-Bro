...
2026-07-09 Phase 4 第三十二刀进行中：继续拆 `server/postgres_cache_store.py` 的只读 PG WebSim read-model selector 边界，新增 `server/pg_gear_read_model_selectors.py::build_websim_loot_read_model()`，仅移动 `get_websim_loot()` 中已构建 loot items、instances 和 season metadata 到 `/api/websim/loot` response envelope 的只读组装逻辑，并保留旧 `items[:limit]` 切片语义。store 保留 active season gate、loot SQL、loot item selector 调用、instances 读取、cache write/sync/write/backfill；公开 observed-only 入口、baseline 默认空、recommended/season/internal prototype 语义不变。当前红测、目标验证、owner-map 合同测试、完整本地验证和本地 CR 已完成；Harness v0.5 candidate deployment gate、PR merge 和 main sync 待完成。
