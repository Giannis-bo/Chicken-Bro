...
2026-07-09 Phase 4 第二十六刀进行中：继续拆 `server/postgres_cache_store.py` 的只读 PG WebSim read-model selector 边界，新增 `server/pg_gear_read_model_selectors.py::build_websim_assets_read_model()`，仅移动 `get_websim_assets()` 中 asset registry rows 到 assets、status/source counts 与 empty/verified status 的只读组装逻辑。store 保留 asset filter mapping、SQL query、limit guard、PG unavailable blocked response、cache write/sync/write/backfill；公开 observed-only 入口、baseline 默认空、recommended/season/internal prototype 语义不变。当前红测、目标验证、完整本地验证和本地 CR 已完成，Harness v0.5 candidate deployment gate 待执行。
