...
2026-07-09 Phase 4 第二十五刀进行中：继续拆 `server/postgres_cache_store.py` 的只读 PG gear/WebSim read-model selector 边界，新增 `server/pg_gear_read_model_selectors.py::build_websim_instances_read_model()`，仅移动 `get_websim_instances()` 中 instance rows 与 encounter rows 到 structured instances list 的 encounter grouping 与字段组装逻辑。store 保留 active season gate、instances/encounters SQL、空表短路、cache write/sync/write/backfill；公开 observed-only 入口、baseline 默认空、recommended/season/internal prototype 语义不变。当前红测、目标验证、完整本地验证和本地 CR 已完成，Harness v0.5 candidate deployment gate 待执行。
