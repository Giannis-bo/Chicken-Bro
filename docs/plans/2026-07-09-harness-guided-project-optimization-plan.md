...
2026-07-09 Phase 4 第二十四刀进行中：继续拆 `server/postgres_cache_store.py` 的只读 PG gear/WebSim read-model selector 边界，新增 `server/pg_gear_read_model_selectors.py::build_websim_loot_items_read_model()`，仅移动 `get_websim_loot()` 中 loot rows 到 normalized loot item list 的 JSON 解析、`normalize_gear_item`、game asset 注入、filters 和 limit 组装逻辑。store 保留 active season gate、loot SQL、instances 查询、response envelope、cache write/sync/write/backfill；公开 observed-only 入口、baseline 默认空、recommended/season/internal prototype 语义不变。当前红测、目标验证、完整本地验证和本地 CR 已完成，Harness v0.5 candidate deployment gate 待执行。
