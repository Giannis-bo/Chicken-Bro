...
2026-07-10 Phase 4 第三十七刀候选推进：继续拆 `server/postgres_cache_store.py` 的只读 PG cache read-model selector 边界，新增独立 `server/pg_cache_read_model_selectors.py::build_raiderio_cache_read_model()`，仅移动 `get_raiderio_payload()` 已读 row 到 cache payload 的 missing-cache blocked envelope、JSON 解析、timestamp 与 sourceStatus defaults。store 保留 Raider.IO cache SQL/cache key、`save_raiderio_payload()` INSERT/ON CONFLICT、sync/observed gear backfill；公开 observed-only gear 入口、baseline 默认空、recommended/season/internal prototype 语义不变。characterization red/green、目标 cache/season/store tests、owner-map contract red/green、完整本地验证和本地 CR 已完成；Harness v0.5 Candidate Deployment Gate、PR merge 和 main sync 待按 evidence packet 晋级。
