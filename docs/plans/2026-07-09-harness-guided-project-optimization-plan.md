...
2026-07-09 Phase 4 第二十三刀进行中：继续拆 `server/postgres_cache_store.py` 的只读 PG/admin gear read-model selector 边界，新增 `server/pg_gear_read_model_selectors.py::build_admin_gear_variant_records_read_model()`，仅移动 `admin_gate_gear_variant_records()` 和 `admin_gate_gear_variant_records_page()` 中 variant row 到后台 `gearVariants` record 的 JSON/字段组装逻辑。store 保留 admin gate SQL、分组分页 SQL、source label subquery、cache write/sync/write/backfill；公开 observed-only 入口、baseline 默认空、recommended/season/internal prototype 语义不变。当前红测、目标验证、完整本地验证和 Harness v0.5 candidate deployment gate 已完成，PR merge/main sync 待执行。
