...
2026-07-09 Phase 4 第十四刀进行中：继续拆 PG gear row/read-model 的只读 mod option type grouping fragment，新增 `build_gear_mod_options_by_type_read_model()`，把 `cache.websim_gear_mod_options` row 按 socket/enchant/embellishment 分类并委托 slot-level option read model 的逻辑从 `server/postgres_cache_store.py` 收敛到 `server/pg_gear_read_model_selectors.py`。本刀只移动只读 row-to-read-model 组装，不移动 PG SQL 读取、cache fingerprint、cache put、sync/write/backfill、template public selector 或公开 observed-only 入口；option display/config policy 仍委托 `websim_payload` 权威 helper。红测已证明缺 helper，最小实现后新 helper 单测和目标 selector/store tests 通过；owner-map 合同测试已按第十四刀先红后绿；完整本地验证 `python3 -m unittest discover -s tests -p '*_test.py'` 为 `1023 tests OK (skipped=1)`，`node --test tests/*.test.js` 为 `344 pass`，`py_compile`、`node --check`、JSON validation、`git diff --check` 和本地 CR 通过。PR branch Candidate Deployment Gate、合入与 post-merge evidence 待执行。
