...
2026-07-09 Phase 4 第十五刀本地推进：继续拆 PG gear read-model 的只读 season catalog candidate grouping fragment，新增 `build_season_recommended_catalog_candidates_by_slot_read_model()`，把 `season_recommendation` 内部构建时从 catalog item read model 派生按槽候选池的合法性过滤、去重、排序和 24 条 limit 从 `server/postgres_cache_store.py` 收敛到 `server/pg_gear_read_model_selectors.py`。本刀只移动只读内存 read-model 组装，不移动 PG SQL 读取、active season 读取、raw row cache、cache fingerprint、cache put、sync/write/backfill、template public selector 或公开 observed-only 入口；`season_recommendation` 仍不能回流公开 `communityTemplates` / `baselineTemplates`。红测已证明缺 helper，最小实现后新 helper 单测和目标 selector/store tests 通过；owner-map 合同测试已按第十五刀先红后绿；完整本地验证 `python3 -m unittest discover -s tests -p '*_test.py'` 为 `1024 tests OK (skipped=1)`，`node --test tests/*.test.js` 为 `344 pass`，`py_compile`、`node --check`、JSON validation、`git diff --check` 和本地 CR 通过。PR branch Candidate Deployment Gate 与合入归档待执行。
