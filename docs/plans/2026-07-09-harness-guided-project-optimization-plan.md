...
2026-07-09 Phase 4 第二十一刀进行中：继续拆 PG/admin gear template read path 的只读 queue row assembly fragment，新增 `build_admin_gear_template_queue_rows_read_model()`，把 `admin_gate_queue_summary()` 中 community gear template rows + official metadata 到 `gear_templates` queue summary rows 的 blocker 组装从 `server/postgres_cache_store.py` 收敛到 `server/pg_gear_template_selectors.py`。本刀只移动 admin/health read-model 的只读 row assembly，不移动 admin gate SQL、`to_regclass` 表存在判断、官方 metadata SQL 查询、offhand coverage repair、recommended/season prototype build paths、cache fingerprint、cache put、sync/write/backfill、cleanup 或公开 observed-only 入口；`server/postgres_cache_store.py` 继续负责查询 adapter 并把 offhand repair 作为 coverage adapter 回调传入 helper。红测已证明缺 helper，最小实现后新 helper 单测和目标 selector/store tests 通过；owner-map 合同测试已按第二十一刀先红后绿；完整本地验证 `python3 -m unittest discover -s tests -p '*_test.py'` 为 `1030 tests OK (skipped=1)`，`node --test tests/*.test.js` 为 `344 pass`，`py_compile`、`node --check`、JSON validation、`git diff --check` 和本地 CR 通过。Candidate Deployment Gate 仍需在 PR 合入前完成。
