...
2026-07-10 Phase 4 第三十九刀已合入：继续拆 `server/postgres_cache_store.py` 的只读 PG cache read-model selector 边界，新增 `server/pg_cache_read_model_selectors.py::build_stat_weight_latest_run_read_model()`，仅移动 `latest_stat_weight_run_payload()` 已读 rows 到 latest-run summary 的 accepted/blocked 计数、status/sourceStatus、lexicographic `refreshedAt` 与 empty-cache blocked envelope。store 保留 PostgreSQL connection、SQL、`ORDER BY computed_at DESC`、`LIMIT 400`、`fetchall()` 和全部 stat-weight save/sync/backfill/cleanup；公开 observed-only gear 入口、baseline 默认空、recommended/season/internal prototype 语义不变。characterization red/green、`"9"` / `"10"` mutation RED、目标 tests、owner-map contract、完整本地验证、本地 CR、整分支 review 和 `4971` 组 base-to-head 等价样例均通过。PR #48 exact head `853c0dd` 已完成 Harness v0.5 Candidate Deployment Gate：标准 health/season/WebSim/stat-weight API、40-spec active observed-only gear sweep、systemd/timer、PG-only runtime、候选窗口日志和 5-file hash parity 均通过。部署未进入 async sync 启动块；既有 enabled `wow-gear-observed-backfill.timer` 于候选窗口按计划运行 6 秒并成功退出，作为独立 scheduled write 记录。PR #48 已 rebase merge 到 `main@5be9d81` 并同步到本地。

2026-07-10 Phase 4 第四十刀已合入：继续拆 `server/postgres_cache_store.py` 的只读 PG/admin read-model selector 边界，新增 `server/pg_cache_read_model_selectors.py::build_admin_gate_queue_summary_read_model()`，仅移动 `admin_gate_queue_summary()` 已组装 rows 到 queue count、domain counts 与 top-8 blockers 的纯聚合。store 保留 admin SQL、table check、official metadata SQL、offhand coverage repair；`server/news_backend.py` 保留授权与跨 store 合并；所有 sync/write/backfill/cleanup/timer 及公开 observed-only gear 语义不变。characterization red/green、store delegation、owner-map contract、目标 `110 tests`、完整 `1055 Python / 344 Node`、10,000 组等价样例、语法/JSON/diff 和 local CR 均通过。PR #49 exact head `e925aab` 已完成 Harness v0.5 Candidate Deployment Gate：授权 admin queue summary、标准 health/season/WebSim API、40-spec active observed-only sweep、systemd/timer、PG-only runtime、候选窗口日志和 5-file hash parity 均通过。部署未进入 async sync 启动块；既有 data-health follow-up timer 在部署后独立触发 observed backfill 与 websim/stat-weight sync，作为 timer backflow 单独记录。PR #49 已 rebase merge 到 `main@a2aa378` 并同步到本地。

2026-07-10 Phase 4 第四十刀后边界审计已完成：`postgres_cache_store.py` 剩余 `admin_gate_gear_records()` 等 envelope 仅做薄字段透传，继续抽取不再降低实质复杂度；`get_sync_state()` 属于 read/write 共属的 `sync_state_repository`，进入该边界会越过本轮不改 sync/write 的批准范围。当前 PG cache/admin read-only selector 纵切在此收口；下一阶段需要单独选择 sync-state repository、其他 backend hotspot 或结构收益复盘，不再自动追加机械 helper。
