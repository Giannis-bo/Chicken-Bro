...
2026-07-10 Phase 4 第三十六刀候选推进：继续拆 `server/postgres_cache_store.py` 的只读 PG active-season read-model selector 边界，新增独立 `server/pg_season_read_model_selectors.py::build_active_season_read_model()`，仅移动已读 active-season row 与 dungeon rows 到 structured season payload 的 cached-payload fallback、字段组装、expiry stale、source refs、dungeon payload 和 raid-pool normalization。store 保留 active-season/dungeon SQL、`active = TRUE` + `data_status = 'verified'` gate、cache write/sync/write/backfill；公开 observed-only gear 入口、baseline 默认空、recommended/season/internal prototype 语义不变。characterization red/green、目标 helper/store tests、owner-map contract red/green、完整本地验证和本地 CR 已完成；Harness v0.5 Candidate Deployment Gate、PR merge 和 main sync 待按 evidence packet 晋级。
