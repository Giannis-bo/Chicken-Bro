...
2026-07-09 Phase 4 第二十九刀进行中：继续拆 `server/postgres_cache_store.py` 的只读 PG WebSim read-model selector 边界，新增 `server/pg_gear_read_model_selectors.py::build_websim_bootstrap_read_model()`，仅移动 `get_websim_bootstrap()` 中已读 season/classes/slots/scenarios/instances/sync/defaultSelection/SimC version 到 bootstrap structured payload 的只读组装逻辑。store 保留 active season 读取、instances/defaultSelection/syncState 调用、fallback sync-state 决策、cache write/sync/write/backfill；公开 observed-only 入口、baseline 默认空、recommended/season/internal prototype 语义不变。当前红测、目标验证、owner-map 合同测试、完整本地验证、本地 CR 和 Harness v0.5 candidate deployment gate 已完成；PR merge 和 main sync 待执行。
