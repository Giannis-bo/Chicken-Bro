...
2026-07-09 Phase 4 第三十一刀进行中：继续拆 `server/postgres_cache_store.py` 的只读 PG WebSim read-model selector 边界，新增 `server/pg_gear_read_model_selectors.py::build_websim_talent_import_template_read_model()`，仅移动 `get_websim_talent_import()` 中已选 verified community talent template row 到 talent import template payload 的字段组装逻辑。store 保留 active season gate、class/spec/hero normalization、talent import SQL、no-row fallback response、cache write/sync/write/backfill；公开 observed-only 入口、baseline 默认空、recommended/season/internal prototype 语义不变。当前红测、目标验证、owner-map 合同测试、完整本地验证、本地 CR 和 Harness v0.5 candidate deployment gate 已完成；PR merge 和 main sync 待执行。
