...
2026-07-10 Phase 4 第三十四刀进行中：继续拆 `server/postgres_cache_store.py` 的只读 PG WebSim read-model selector 边界，新增 `server/pg_gear_read_model_selectors.py::build_websim_community_talent_templates_read_model()`，仅移动 `_community_talent_templates()` 中已筛选 PG rows 到 structured community talent template list 的 JSON/int 解析、字段映射、source ref normalization、可应用状态与既有 spec/hero slot selection 组装逻辑。store 保留 community talent template SQL、verified/expiry 过滤、排序/limit、active season/sync state 读取、talent SQL、cache write/sync/write/backfill；公开 observed-only gear 入口、baseline 默认空、recommended/season/internal prototype 语义不变。当前 characterization red/green、目标 selector/store tests、owner-map contract red/green、完整本地验证、本地 CR 和 Harness v0.5 candidate deployment gate 已完成；PR merge 和 main sync 待完成。
