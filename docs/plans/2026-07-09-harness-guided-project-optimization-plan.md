...
2026-07-09 Phase 4 第三十刀进行中：继续拆 `server/postgres_cache_store.py` 的只读 PG admin/WebSim read-model selector 边界，新增 `server/pg_gear_read_model_selectors.py::build_admin_talent_records_read_model()`，仅移动 `admin_gate_talent_records()` 中已读 community talent template rows 与 talent tree rows 到后台 admin talent record payload 的字段组装和 expired-template 过滤逻辑。store 保留 admin SQL、table/cache reads、cache write/sync/write/backfill；公开 observed-only 入口、baseline 默认空、recommended/season/internal prototype 语义不变。当前红测、目标验证、owner-map 合同测试、完整本地验证和本地 CR 已完成；Harness v0.5 candidate deployment gate、PR merge 和 main sync 待执行。
