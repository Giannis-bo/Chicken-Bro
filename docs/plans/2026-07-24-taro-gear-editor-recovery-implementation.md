...
- 小程序 transport 对 `/api/websim/gear` 始终消费 compact payload；不得用
  `compact:false` 假设可取得 item-level 完整详情。slot hydration 必须从真实
  `replacementCandidates[]` group 读取三类 option arrays 并安全投影到 exact
  items；没有 group 时 fail closed。
- 页面不得推断等级轨道、socket 数、唯一宝石、美化上限、来源质量或最终属性；只消费 `mode=slot` 返回的候选、`variants` 和具有 canonical `id/optionKey` 的 option fields。
- 每次候选/强化确认均调用 canonical `gearResolve`；只有 `status=resolved`、snapshot `verified`、`resolvedSlots` identity 完整匹配且 domain snapshot helper 可物化确认强化时才能提交本地已确认状态。
- `gemOptionIds` 遵循现有 Resolver packed-order contract：前端草稿、intent 与已确认强化都不得保存空位；`socketCount` 单独作为显示容量，移除前位时后续宝石前移。
- compact `craftedStatOptions` 位于 `variants[]`，`simcOptions` 为对象且没有 Resolver 可校验的 ID；只能跟随当前所选 variant 做安全只读展示，不能映射为 `craftedOptionId` 或自造 `variantKey`。若此约束必须改变，停止本计划并按 Strict 开立后端 contract 任务。
...
| 文件 | 责任 |
| --- | --- |
| `apps/mini-taro/src/pages/builds/gear-detail-editor-model.ts` | 安全解析候选轨道与 variant-scoped display-only 制造属性、共享 eligibility、候选草稿/应用投影、packed-order 宝石和单值增强草稿变更；不发请求。 |
| `apps/mini-taro/src/pages/builds/gear-detail-editor-model.test.ts` | 轨道、候选确认、强化草稿和失败不提交的纯函数回归。 |
| `apps/mini-taro/src/pages/builds/gear-detail-editor-commit-model.ts` | 从 verified `resolvedSlots` 校验最终 identity、调用 domain snapshot helper 物化 confirmed enhancements，并处理失败、409 与 stale completion。 |
| `apps/mini-taro/src/pages/builds/gear-detail-editor-commit-model.test.ts` | 草稿不提前提交、missing/mismatch/partial snapshot fail closed、resolved item/variant/selectedOptions 提交、失败/409/stale 不污染已确认状态的行为回归。 |
...
| `apps/mini-taro/src/pages/builds/gear-detail-model.ts` | compact slot group hydration、canonical enhancement option 过滤、完整多 socket 已选态投影和玩家文案“美化”。 |
| `apps/mini-taro/src/pages/builds/gear-detail-model.test.ts` | 真实 compact group fixture、unsafe option blocker、多 socket 选择态与“美化”摘要的显示合同。 |
...
- Consumes: `GearItemReference`、`GearEnhancementSelection` 和后端字段 `variants[].craftedStatOptions[]`（兼容 candidate 顶层旧展示字段）。
...
解析 `variants` 时只读取 array/object/string/number 的安全值，过滤 `needs-variant` 占位符，保留后端 `key|variantKey`、`difficultyLabel`、`itemLevel|ilevel`、状态和 blockers。选择轨道后只投影该 variant 的 `craftedStatOptions`；对象型 `simcOptions` 仅把安全标量格式化为稳定字符串。`materializeCandidateDraft` 必须合并所选轨道的 item level、variant key、verified display fields 和 candidate 的 option arrays；不能把 `craftedStatOptions[].key`、`simcOptions` 或任意未知字段转换为 `craftedOptionId`。
...
- Consumes: Task 2 model、Task 3 sheets、真实 compact
  `wowApi.websim.gear({ compact:true, mode:'slot', slot })` group、
  `GearRequestFence`、`gearResolve` 和 domain
...
写失败的纯行为测试并要求页面消费该 helper：未 Resolve、network/503 或 stale completion 时 candidate/enhancement draft 与已确认 `equipped` / `enhancements` 都保持；409 只清两个草稿并要求 reload；verified candidate completion 必须从 `resolvedSlots` 校验目标 `itemId + variantKey`，并由 domain helper 从同一 snapshot 物化 confirmed enhancements；verified enhancement completion 同样只提交 `resolvedSlots[].selectedOptions`。missing/mismatch/partial snapshot 全部 fail closed。无 `variants` 的后端合法候选仍产生 base materialization、显示“此候选无需选择等级轨道”并保留显式 Apply；交互 verifier 只在返回 variants 时选择轨道。

再为真实微信 smoke 写失败断言：每个 `gear-slot-row` 必须发布 `data-committed-item-id`，候选详情发布 selected returned variant key，工作台发布 `data-gear-resolve-state`、resolved item/variant 和 committed variant markers。`verify-ui-interactions.js` 在 apply 前确认 item 未提交；点击 apply 后必须等待本次 resolving → verified，并要求 committed/resolved 的 itemId 与 selected candidate 相同、committed/resolved 的 variantKey 与真正点击的 returned variant 相同。

- [ ] **Step 2: 用 slot hydration 生成编辑输入**

将 `chooseSlot` 抽为“打开候选 sheet + 请求/缓存该 slot detail”；只有返回真实 slot group 才把 group-level `socketOptions/enchantOptions/embellishmentOptions`（未提供视为后端声明的空可选集）投影到每个 exact item 并缓存。打开强化条时复用相同 hydration，按 itemId 和已确认 variantKey 找精确候选；没有 group/exact item 或请求分组没有 canonical option identity 时展示 blocker 并禁用确认。
...
候选 row、轨道和强化 option 只调用 `set*Draft`。`confirmEnhancementDraft` 以 packed 草稿执行一次 Resolve，成功后必须由 domain helper 从 verified `snapshot.resolvedSlots` 读取并提交强化，不能提交原始 draft、请求 intent 或 `snapshot.selectionIntent`。503/network 或缺失 verified slot 仅保留草稿和已确认 state；409 清草稿并 reload；stale completion 不更新任何 state。
...
`data-gear-resolve-state` 在加载期间必须为 `resolving`，不能复用旧 snapshot 的 `verified`；额外发布只来自 `resolvedSlots` 的 current resolved item ID / variant key，以及当前 committed variant key。交互执行器按 selected returned variant 同时核验 resolved/committed item 和 variant，旧 verified snapshot、请求 intent 或乐观提交都不能伪造本次成功。
...
- Spec coverage: Task 2 覆盖 compact group hydration、候选轨道、variant-scoped 制造属性、canonical option filtering、packed-order 多 gem 与“美化”显示；Task 3 覆盖独立 socket 容量和当前 UI 内编辑控件；Task 4 覆盖 `resolvedSlots` identity/enhancement proof、Resolver 成功/失败/409/fence；Task 1/5 覆盖合同、release、验证与人工验收。
