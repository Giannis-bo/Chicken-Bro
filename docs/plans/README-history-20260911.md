...
本文件保留整理前原文，不代表当前状态或新增授权。当前入口见[README.md](README.md)。历史链接保持原相对位置，按具体证据需要读取，不作为日常执行必读。

---

# 计划白名单

- `正在推进 / 已授权实现并发布`：[通用研究证据与语义优化](2026-09-11-research-generalization.md)。五项优化、固定评测及隔离/线上验证。

- `已完成 / 已发布`：[有限研究完整解答](2026-09-11-research-completion.md)：跨轮证据复用、治疗汇总、正确计费和执行保护。
...
- `已完成 / 持续执行规则已落地`：[Badcase 自动修复工作流](2026-09-08-badcase-workflow.md)。定义通用工具修复、有界验证及条件发布闭环；命令与证据结构见[执行说明](../badcase-workflow-operations.md)，历史个案见[归档](2026-09-08-badcase-workflow-history.md)。具体批次状态按持久记录核验，不以流程落地代表业务验收。
