...
- Task 5A 的用户确认设计和 `requirement.json` 位于 `2026-08-07-equipment-simulator-exact-first-task5a-api-runtime.md` 与 `artifacts/releases/2026-08-07-equipment-simulator-exact-first-task5a/requirement.json`；其当前状态为 `implementation_allowed`，只允许该 plan 冻结的本地 TDD 实现。当前 v1 source 没有可验证 Authority Bundle mapping，故 route/UI binding、candidate、deployment 或生产状态仍不得推进为 ready；Task 6C 仍须自行补出全部 task-scoped allowlist、用户可见验收、candidate/rollback 和独立 review；本计划当前只给顺序和边界，不提前授权其实现。
