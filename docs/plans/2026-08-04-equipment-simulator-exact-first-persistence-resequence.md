...
状态：`正在推进（Task 3A 的 t3a2608050955 与 t3a260805111623 是 0003 migration-chain failed runs；t3a260805113656 fresh/upgrade runtime passed，但 active requirement.status 错为 local_verified，Harness 禁止证据晋升，且 candidate 后 requirement/owner correction 使该 run 失效。当前 literal candidate_rerun_required / evidence_promotion_blocked；三对六库 immutable/non-reusable；必须以第四个新 run-id 和新双库重跑；未生成 evidence/manifest；无 runtime consumer）`
...
- [ ] Candidate PostgreSQL evidence from both explicit DSNs is mandatory before Task 3A is `已完成`. `t3a260805113656` passed runtime but is unpromotable because the requirement gate was not `implementation_allowed`; the subsequent control-plane correction invalidates that run. Current status is literal `candidate_rerun_required / evidence_promotion_blocked` until a fourth new run-id/new fresh-upgrade pair passes on the final committed head; one database, a shared development database, skipped tests, schema-only mocks or any prior run-id cannot be packaged as green.

`t3a2608050955` 与 `t3a260805111623` 已在 `0003` 前失败；`t3a260805113656` 在 runtime 层通过，但 active requirement gate 错误使它不可晋升，且本次 requirement/owner correction 使其失效。当前 checkout 必须保持 literal `candidate_rerun_required / evidence_promotion_blocked`，不得生成或晋升 `evidence.json`/`manifest.json`。六座旧库——`wow_exact_first_fresh_test_t3a2608050955`、`wow_exact_first_upgrade_test_t3a2608050955`、`wow_exact_first_fresh_test_t3a260805111623`、`wow_exact_first_upgrade_test_t3a260805111623`、`wow_exact_first_fresh_test_t3a260805113656`、`wow_exact_first_upgrade_test_t3a260805113656`——只保留为 immutable/non-reusable 证据。第四次 candidate 必须使用新 run id 和两座新建、独立、空的 operator-provisioned 数据库，并验证恰有一个 named config-hash target constraint、`contype = 'u'`、`user_id/template_type/config_hash` 有序列、无 legacy name constraint，以及恰有一个 `0003` ledger row。

2026-08-05 首轮独立实现 review 返回 `CHANGES_REQUIRED`；deterministic document insertion、exact import allowlist、absent-row reverse-shared-record concurrency、精确 SQL/ACL/upgrade parity、单行 candidate attestation 与 closed-universe static gates 等 findings 已完成 RED/GREEN correction。Task 3A code/test state 及最终 `0003` semantic-idempotence/type correction 均经 fresh scoped reviewer 复审为 `PASS/APPROVED`、0 findings；当前阻塞仅是 requirement gate 修正后的第四次真实双 PostgreSQL candidate 与随后 evidence/manifest 晋升，不能复用 `t3a260805113656` 的 runtime pass。
