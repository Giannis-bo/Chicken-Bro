...
状态：`正在推进（Task 3A/3B/4P/4L/4W 的交付均已归档；Task 3A 的最终候选为 t3a260805163536，正式 packet 保持 runtime_verified / candidate_verified、closure identity 为 pending。Task 5C Runtime Authority 候选已验证，production foundation 已将 0030--0035 以 runtime_verified 落入正式 PostgreSQL；provider/worker 保持 disabled，aggregate readiness 的 eligible source 为零。因此当前 ready path 仍 literal blocked，不创建任务；任何 source admission、activation、Task 6C 或真实微信验收都必须由新的 task-scoped contract 授权。）`
...
- Task 4L 的 Strict pure-domain slice 已归档；其 owner、aggregate schema 与完整性证明只保留在 release packet 和 Git 历史。随后 Task 3B 的独立 requirement/replay-context review 已将执行严格限定在下述 0031 persistence allowlist；该历史归档不扩展至后续 runtime 或用户可见动作。
...
- Task 5A/5B 的 local foundation 与 source-gap 分析已由后续 Task 5C 收敛。当前可执行边界只有 [Task 5C Runtime Authority Release 设计](2026-08-10-equipment-simulator-exact-first-task5c-runtime-authority-release-design.md)：它使用 project-owned、只覆盖闭合模板的 immutable release 与 forward-only v3 identity；`0033` 仍只重放 source-to-slot-bundle closure，`0031` 仍只保存已完成 aggregate。Task 5C 候选和 production foundation 已分别记录在其 release packet；首 source aggregate 为零，provider/worker 继续 disabled，ready-path literal `blocked`。任何 activation、Task 6C 或既有页面的真实微信验收都必须开立新的 task-scoped contract。
...
- Historical predecessor plans are removed from the working tree; their release evidence and Git history retain the original scope and status.
