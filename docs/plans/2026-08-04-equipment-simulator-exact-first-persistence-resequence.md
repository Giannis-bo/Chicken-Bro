...
- Task 5A 的用户确认设计和 `requirement.json` 位于 `2026-08-07-equipment-simulator-exact-first-task5a-api-runtime.md` 与 `artifacts/releases/2026-08-07-equipment-simulator-exact-first-task5a/requirement.json`；其 local TDD foundation 已完成，但 ready-path 仍不得推进。后续 [Task 5B authority-source contract](2026-08-07-equipment-simulator-exact-first-task5b-active-authority-source.md) 处于 `requirement_challenged`：0033 只能重放 source-to-slot-bundle closure，0031 只能保存已完成 aggregate，现无 immutable eight-field runtime release 或 occurrence index。任何 provider、migration、candidate、deployment 或生产状态均须由新的 independently allowlisted task 重新授权；Task 6C 仍须自行补出全部 task-scoped allowlist、用户可见验收、candidate/rollback 和独立 review；本计划当前只给顺序和边界，不提前授权其实现。
