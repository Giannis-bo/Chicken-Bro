...
状态：`正在推进（Task 3A implementation_allowed / candidate_pending。第七次 t3a260805160003 从 exact frozen 8c9490cd / tree f71aad19 真实通过 migration 0030 fresh/upgrade 与独立只读审计，但 exact-source full profile 随后因两份状态文档缺少 literal 0030、current-truth test 硬编码 implementation_allowed 而失败；状态为 runtime_passed_unpromotable_current_truth_lifecycle_test_regression。runtime/verification/closure identities pending，七个 run 的十四库全部 immutable/non-reusable，必须第八次新 run-id/双库重跑。literal candidate_rerun_required / evidence_promotion_blocked 仍是当前工程真相；无 production migration/runtime consumer，Task 4P+ 未授权）`
...
- [ ] current evidence is `implementation_allowed / candidate_pending`; an eighth candidate needs a new run-id and two new databases, and `candidate_rerun_required / evidence_promotion_blocked` remains current engineering truth.

`t3a2608050955` 与 `t3a260805111623` 已在 `0003` 前失败；`t3a260805113656` 因 requirement gate 不可晋升；第四次 `t3a260805120026` 为 `runtime_passed_superseded_by_main_integration`；第五次 `t3a260805125812` 为 `runtime_passed_invalidated_by_direct_runtime_import_regression`；第六次 `t3a260805142130` 为 `runtime_passed_unpromotable_evidence_lifecycle_test_regression`；第七次 `t3a260805160003` 为 `runtime_passed_unpromotable_current_truth_lifecycle_test_regression`。十四座库全部 immutable/non-reusable；第八次新 run-id/双库 mandatory，且 migration `0030` 无 production migration/runtime consumer。

2026-08-05 第七次 candidate 真实通过 0030 与独立只读审计，但 frozen HEAD `8c9490cd` 的 exact full profile RED 证明其 current-truth source 不是 full-profile green。当前证据回退为 `implementation_allowed / candidate_pending`；runtime/verification/closure identities pending，第八次 candidate mandatory，`candidate_rerun_required / evidence_promotion_blocked` 未解除。
