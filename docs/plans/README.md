...
| 装备模拟 Exact-first 与三来源 Catalog | 正在推进（Task 3A `implementation_allowed / candidate_pending`。第四次 `t3a260805120026` 保持 `runtime_passed_superseded_by_main_integration` 历史；第五次 `t3a260805125812` 的 exact 0030 PostgreSQL attestation-v2 是 immutable 历史，但 PR #114 run `30977816268` / job `92215539827` 的 direct-server import regression 使其为 `runtime_passed_invalidated_by_direct_runtime_import_regression`，不是当前 promotion。literal `candidate_rerun_required / evidence_promotion_blocked` 为当前工程真相；五个 run 的十库 immutable/non-reusable，controller full local profile 后必须第六次新 run-id/双库。无 production migration/runtime consumer，Task 4P+ 未授权） | [设计](2026-08-04-equipment-simulator-exact-first-catalog-design.md) · [原实施计划](2026-08-04-equipment-simulator-exact-first-implementation.md) · [持久化与运行链重排](2026-08-04-equipment-simulator-exact-first-persistence-resequence.md) · [Task 3A evidence](../../artifacts/releases/2026-08-04-equipment-simulator-exact-first/evidence.json) |
