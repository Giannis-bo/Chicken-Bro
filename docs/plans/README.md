...
- `正在推进`：[原生搜索恢复](2026-09-10-bounded-research.md#2026-09-10-原生搜索恢复正在推进)，源码已合入；隔离通过、线上语义失败并回滚，保留后续核验待办。
...
- `已完成 / 后端与 Web 已发布，Mini 免验未上传`：[鸡哥截图提问](2026-09-09-chat-images.md)，双端图片输入、私有存储和真实模型视觉。
