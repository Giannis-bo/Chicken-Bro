...
| 装备模拟 Exact-first 与三来源 Catalog | 正在推进（Task 3A `implementation_allowed / candidate_pending`。第四次 `t3a260805120026` 为 `runtime_passed_superseded_by_main_integration`；第五次 `t3a260805125812` 为 `runtime_passed_invalidated_by_direct_runtime_import_regression`；第六次 `t3a260805142130` 从 exact frozen `9f67ec2f` / tree `c2a686e6` 真实通过 0030 PostgreSQL 和独立只读审计，但 frozen HEAD 的 owner current-truth test 硬编码 `implementation_allowed`、无法容纳已授权 `local_verified` transition，故为 `runtime_passed_unpromotable_evidence_lifecycle_test_regression`。literal `candidate_rerun_required / evidence_promotion_blocked` 仍为当前工程真相；六个 run 的十二库 immutable/non-reusable，必须第七次新 run-id/双库。runtime/verification/closure identities pending，无 production migration/runtime consumer，Task 4P+ 未授权） | [设计](2026-08-04-equipment-simulator-exact-first-catalog-design.md) · [原实施计划](2026-08-04-equipment-simulator-exact-first-implementation.md) · [持久化与运行链重排](2026-08-04-equipment-simulator-exact-first-persistence-resequence.md) · [Task 3A evidence](../../artifacts/releases/2026-08-04-equipment-simulator-exact-first/evidence.json) |
