...
- `正在推进 / 工作流已实现，真实回放与发布验收中`：[Badcase 本地复盘与人工决策](2026-09-08-badcase-workflow.md)，用户认可定期扫描、对照验证、根因归组和通用回归；每批汇总后由用户按问题组确认处理方向，heartbeat 已配置；本轮已授权落地扫描、诊断、版本绑定批准及条件发布，并修复发布两条排行榜反馈，真实验收进行中。
