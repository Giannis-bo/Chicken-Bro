...
| 装备模拟 Exact-first 与三来源 Catalog | 正在推进（Task 3A `candidate_rerun_required / evidence_promotion_blocked`：第四次 `t3a260805120026` 真实通过旧 0026 链，但 origin/main 已占用 0026-0029，Task 3A 必须顺延到 0030，因此该 run 为 `runtime_passed_superseded_by_main_integration`。四个 run 的八库 immutable/non-reusable；完成主线集成与本地验证后最高仅 `local_verified / candidate_pending`，必须以第五个新 run-id 和两座新库重跑方可恢复 `runtime_verified`；无 production migration/runtime consumer，Task 4P+ 未授权） | [设计](2026-08-04-equipment-simulator-exact-first-catalog-design.md) · [原实施计划](2026-08-04-equipment-simulator-exact-first-implementation.md) · [持久化与运行链重排](2026-08-04-equipment-simulator-exact-first-persistence-resequence.md) · [Task 3A evidence](../../artifacts/releases/2026-08-04-equipment-simulator-exact-first/evidence.json) |
