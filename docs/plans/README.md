...
| 装备模拟 Exact-first 与三来源 Catalog | 正在推进（Task 3A `local_verified / candidate_pending`；runtime promotion 仍为 `candidate_rerun_required / evidence_promotion_blocked`。origin/main 已在 `1619815c` 集成，主线 0026-0029 与 Task 3A 0030 共存；365/519/40/25 测试矩阵及独立 whole-branch CR 通过。第四次 `t3a260805120026` 是真实但 superseded 的旧 0026 历史；四个 run 的八库 immutable/non-reusable，必须第五个新 run-id 和两座新库重跑方可恢复 `runtime_verified`；无 production migration/runtime consumer，Task 4P+ 未授权） | [设计](2026-08-04-equipment-simulator-exact-first-catalog-design.md) · [原实施计划](2026-08-04-equipment-simulator-exact-first-implementation.md) · [持久化与运行链重排](2026-08-04-equipment-simulator-exact-first-persistence-resequence.md) · [Task 3A evidence](../../artifacts/releases/2026-08-04-equipment-simulator-exact-first/evidence.json) |
