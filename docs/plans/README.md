...
- `暂缓 / 最终修复验收通过，发布配额门禁未满足`：[Badcase 本地复盘与人工决策](2026-09-08-badcase-workflow.md)，用户认可定期扫描、对照验证、根因归组和通用回归；每批汇总后由用户按问题组确认处理方向，heartbeat 已配置；本轮已授权落地扫描、诊断、版本绑定批准及条件发布，并修复发布两条排行榜反馈；最终8项已通过，因发布配额未达预留门槛，按用户要求停止自动迭代，等待仅恢复发布步骤。
