...
| 装备模拟 Exact-first 与三来源 Catalog | 待决策（Task 3A 交付收尾已完成：PR #114 已合入，合并结果及最终归档记录均通过 scoped Harness，main/origin parity 已确认；第八次 `t3a260805163536` 的 disposable 双库已处置、候选产物已可恢复隔离，原实现 worktree/分支已移除，前七个 run 的十四库未触碰且 immutable/non-reusable。其正式 packet 仍为 `runtime_verified / candidate_verified`、closure identity `pending`，因为现行 lifecycle test 不建模 archived。Task 4P 的 final scoped re-review 仍复现 mixed genuine-bundle dependency closure 与 occurrence container/boolean ordinal 的 fail-closed P1；packet 已降为 `implementation_allowed`，分支未合并/推送，等待新的明确修复授权。无 production migration/runtime consumer，generation 35 未改写；4L、3B、4W、5A、6C 和任何用户可见/生产动作仍未授权） | [设计](2026-08-04-equipment-simulator-exact-first-catalog-design.md) · [原实施计划](2026-08-04-equipment-simulator-exact-first-implementation.md) · [持久化与运行链重排](2026-08-04-equipment-simulator-exact-first-persistence-resequence.md) · [Task 3A evidence](../../artifacts/releases/2026-08-04-equipment-simulator-exact-first/evidence.json) · [Task 4P evidence](../../artifacts/releases/2026-08-05-equipment-simulator-exact-first-task4p/evidence.json) |
