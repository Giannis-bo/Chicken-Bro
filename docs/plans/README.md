...
| 装备模拟 Exact-first 与三来源 Catalog | 正在推进（Task 3A 的 `t3a2608050955` / `t3a260805111623` 是 migration-chain failed runs；`t3a260805113656` runtime passed 但因 requirement gate 错误不可晋升。当前 `candidate_rerun_required / evidence_promotion_blocked`；三对六库 immutable/non-reusable，第四个新 run-id/双库 candidate 强制重跑；无 evidence/manifest 或 runtime consumer） | [设计](2026-08-04-equipment-simulator-exact-first-catalog-design.md) · [原实施计划](2026-08-04-equipment-simulator-exact-first-implementation.md) · [持久化与运行链重排](2026-08-04-equipment-simulator-exact-first-persistence-resequence.md) |
