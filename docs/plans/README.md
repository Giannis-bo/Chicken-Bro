...
| 装备模拟 Exact-first 与三来源 Catalog | 正在推进（Task 3A `runtime_verified / candidate_verified`。第八次 `t3a260805163536` 在 runtime commit `9f230f2e` / tree `89e7e5e` 通过 migration `0030` fresh/upgrade、attestation-v2 与两层只读审计；verification commit `1714f376` / tree `7ba9ff18` 通过唯一 173-command full profile（Node 747、Python 2718，含 2 个预期 skip、Vitest 437）及 candidate→verification 精确 owner test + active plan 两路径门禁。历史 PR CI run `30993690037` 的 29 个 Python 3.11 AST 序列化假失败已在该 verification HEAD 纠正，不属于 runtime candidate 失败。runtime/verification identities 已分离绑定，closure 仍 pending corrected PR CI、push、merge、合并后验证、归档与 scoped cleanup。第七次 `t3a260805160003` 仍以 `runtime_passed_unpromotable_current_truth_lifecycle_test_regression` 保留为历史；前七个 run 的十四库 immutable/non-reusable。无 production migration/runtime consumer，generation 35 未改写，Task 4P+ 未授权） | [设计](2026-08-04-equipment-simulator-exact-first-catalog-design.md) · [原实施计划](2026-08-04-equipment-simulator-exact-first-implementation.md) · [持久化与运行链重排](2026-08-04-equipment-simulator-exact-first-persistence-resequence.md) · [Task 3A evidence](../../artifacts/releases/2026-08-04-equipment-simulator-exact-first/evidence.json) |
