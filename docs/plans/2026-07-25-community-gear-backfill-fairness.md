...
- If verified observed variants require a new immutable Gear Release, seal and bind it only as a candidate preview. Recompose against that dependency only during explicit candidate controlled cutover; do not reuse old projections across the dependency boundary or alter the retail Manifest pointer.
...
Run bounded candidate backfill until the persisted cursor passes 皓月当空. Record cursor and stop reason on each run. If the verified observed variants are newer than the active Gear Release, seal a candidate-only Gear Release and bind it through WOW_GEAR_PREVIEW_RELEASE_ID without moving the retail Manifest. Compose observed-build candidate data; if the live ranking payload no longer contains the cited player, recompose the existing same-slot LKG only after regular winner selection has retained it. A Gear Release dependency change requires explicit candidate controlled cutover. Then prove item 258516, level 298, bonus IDs 13440/6652/12701/13654, and enchant IDs 7981/8052 are a verified exact variant.
