...
状态：`对话设计已确认；书面版本待用户复核；未授权实现、候选发布或指针切换`
...
- List A 中所有公开 Item、BrowseVariant、轨道和强化兼容关系必须可生成确定的 SimC 输入。
...
### 4.1 `SelectableCatalogRelease`（List A）

职责：封存一个有限、可浏览、可编辑且全部 SimC-ready 的候选目录。
...
- Catalog builder、规则、Profile Compiler 和固定 SimC runtime identity。

List A 的 membership 由版本化产品准入策略和候选构建器拥有。固定 SimC runtime 负责
materialization 与执行门禁，但不能单独授予官方来源、赛季获取关系或目录完整性声明。
只有存在独立 provenance 的来源文案才能展示为已确认来源；缺少来源证明时不得伪造来源，
但只要产品准入、progression、静态事实和 SimC 合同完整，可以保留“可模拟”这一有界承诺。

### 4.2 `CommunityTemplateSnapshot`

职责：不可变保存 Raider.IO 模板的来源身份、角色/专精上下文、观察时间、原始装备字段和内容
哈希。解析、替换或保存派生模板均不得修改原始快照。

### 4.3 `ImportedExactItemInstance`
...
### 4.5 `ResolvedLoadout` 与 `SimulationSnapshot`
...
| 问题 | Owner | 允许声明 | 禁止升级 |
| --- | --- | --- | --- |
| 哪些装备可在替换列表选择 | `SelectableCatalogRelease` | 当前 List A membership | 社区样本或 SimC 成功自动扩张 List A |
...
| 社区装备能否重放 | Exact Resolver + 固定 SimC runtime | `ready/pending/partial/blocked/stale` | 用 BrowseVariant 或相似装备补值 |
...
本地 SimC 数据/runtime identity。需要外部字段时必须经过独立、显式、版本化输入，不得由一次
SimC 运行临时联网补入并写回 Catalog。
...
3. 模板页面正常显示非 List A 装备、属性和实际强化，并标记“社区导入 · 只读”。
...
社区 Snapshot 与 Exact Registry 不扩张 Manifest；每条记录分别绑定其来源 hash、观察时间、
Resolver 和 SimC runtime。SimC 更新后必须构建和验证新的 List A candidate；旧 List A 保持活动，
不得只替换二进制。
...
1. **S2 List A 候选**：建立有限准入策略、SelectableCatalogRelease、强化兼容边和固定 SimC
   离线全矩阵；不切 Active Manifest，不开放玩家模拟。
2. **社区 Exact 与混合配装**：接入不可变社区 Snapshot、只读 Exact Registry、Mixed Resolver、
   替换/撤销/恢复/派生保存；仍以候选环境验证。
3. **统一 SimC 联动与用户验收**：绑定完整 SimulationManifest、任务执行、状态展示、候选部署、
   四条真实微信路径、原子切换和回滚。

第一阶段需要 SimC 作为离线验证器，但不提前实现或开放第三阶段的玩家任务链。后续实现计划必须
按三个子项目拆分，不能恢复为一个大而全的切换任务。
...
新工作必须从本设计建立独立 task-scoped 计划与候选身份。当前 generation 35、S1 活动链、
provider/worker disabled 状态和全局 `partial` 均保持不变，直到后续书面复核、实施计划、候选验证
和用户验收分别完成。
