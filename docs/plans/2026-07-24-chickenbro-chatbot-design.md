...
状态：`正在推进`
分类：`Strict（分阶段实施）`
确认日期：2026-07-24；补充确认：2026-08-01

实施入口：[Phase 1 极简聊天表面](2026-08-01-chickenbro-chat-surface-design.md)、[聊天表面实施计划](2026-08-01-chickenbro-chat-surface-implementation.md)、[来源驱动 Agent 实施计划](2026-08-01-chickenbro-source-agent-implementation.md)。后续阶段分别规划、验证和发布，不做一次性整体切换。
...
炸鸡队长对玩家只呈现为一个自然聊天的 ChatBot，专注于魔兽世界正式服当前版本和测试服的构筑、SimC、WCL 与社区信息分析。玩家不需要选择“即时问答”“证据模式”或“诊断模式”，也不需要为了普通职业咨询先提交 WCL 链接；云端 Agent 根据问题、当前对话、授权范围和来源可用性自动选择受控 Tool，必要时才追问一个关键对象或把重分析转为可追踪任务，再回到同一段对话给出结论。
...
已确认的“自我更新学习”指来源记忆，而不是擅自修改模型权重：定时同步和每次获准 Tool 查询将带版本、时间与来源的紧凑事实写入既有缓存/读模型；Agent 只把仍在新鲜度窗口内的事实带入下一次回答。聊天原文、私有日志、密钥和模型推断不得自动沉淀为公共知识。
...
普通问题直接按当前可用来源给出快速、自然的回答。比如“防战现在怎么配”会由 Agent 自动查找版本、专精与场景对应的社区来源/缓存，而不是回退成泛泛教程。需要个人数据时，后端先在当前账号范围内寻找候选：只有一个明确对象时直接使用，存在多个模板或任务时返回选择卡片。只有私有 WCL、明确消耗资源的 SimC 对比或可能改变分析对象时，才要求玩家确认；没有 WCL 链接的普通提问不得被阻断。
...
Agent 可以选择受限 Tool、归纳事实和解释取舍，但不能直接访问 PostgreSQL、外部 API、密钥或执行器，也不能自行计算 WCL 事件和 SimC 结果。生产主链使用云端已配置的 Codex Agent 执行器；它的具体 provider 可替换，且不以交互式 CLI、Shell 或本地文件访问作为用户请求的执行通道。后端先校验 Agent 的 Tool 意图，再执行 Tool，并把结构化结果回传给 Agent 组织自然回答。
...
来源记忆是 WoW Source Gateway 的一个读模型职责，不是面向玩家的新功能：

- Raider.IO 的已验证缓存和社区模板同步是首个自动使用的社区来源；普通版本/职业问题可先读取新鲜缓存，过期时按既有同步预算刷新，失败时明确保留时间和限制；
- Warcraft Logs 只通过官方授权 API 查询。公开报告链接出现时可自动检查；私有报告仍要求用户 OAuth 授权。没有报告链接时，Agent 可以继续做社区层面的建议，但不能冒充个人复盘；
- Archon.gg 在得到可用 API 或书面授权前只保留为参考来源，不进行页面抓取或把静态快照描述为实时事实；
- 每个可进入 Agent 上下文的事实都保留来源、引用、游戏版本/赛季、采集时间、状态和失效时间。来源刷新替换的是可追溯事实摘要，不是模型权重，也不会从用户聊天自动学习未经验证的结论。
...
炸鸡队长像一位轻松、专业、有实战经验的玩家正常聊天：回答随问题而定，不套固定的“结论—依据—下一步”模板，也不要求玩家填写 Tool 参数。能够给出当前资料足够支持的建议时直接说人话；资料不够、来源过期或对象不明确时，也直接说明缺口并自然追问。任何具体排名、DPS、数值、个人日志判断和版本结论仍必须来自可追溯的 Tool 结果，不能用流畅语气掩盖来源限制。
