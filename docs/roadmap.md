...
| 正在推进 | Exact-first 与三来源 Catalog 解耦 | Pure Canonical foundation 与 source change-control 已 final PASS/APPROVED；Task 3A delivery closure、Task 4P、Task 4L pure-domain、Task 3B persistence delivery closure 和 Task 4W delivery closure 均已完成。Task 3A 历史 candidate `t3a260805163536` 仍为 `runtime_verified`，closure `pending`。Task 3B candidate `t3b260806203541`、独立 CR、PR #117 CI、merge `61eddf45`、merge-result scoped Harness、main/origin parity 与候选资源清理均已完成；正式 packet 保持 `runtime_verified / candidate_verified` 与 closure `pending`，因为 lifecycle contract 不建模 archived。Task 4W candidate `t4w260807000500`、PR #118 exact-head CI `31119305966`、merge `14f47437`、merge-result scoped Harness、main/origin parity、归档与候选资源清理均已完成；正式 packet 同样保持 `runtime_verified / candidate_verified` 与 closure `pending`。Task 5A 已冻结独立 allowlist，并有 generic Exact confirm/submit/read、0033 source binding/admission/replay 与 injected v2 materializer/worker 的 local TDD foundation；它只会接合当前 `simc_submit` 的 backend binding，不重做 UI 视觉/入口。它只从服务端重读的保存装备模板建立 canonical binding，不依赖 client metadata 或 registry-wide runtime discovery；用户现已确认天赋同样只可通过 authenticated owner 的 remote saved template server reload/compile，而 client raw profile、local/handoff 与未通过服务器 options revalidation 的 race/scenario 全部 literal blocked。用户已批准前向 0034：保留既有 0032/v1 rows 与 canonical bytes，不回写历史；新的 `exact-import-job-request-v2` 将 `resolvedLoadoutKey`、`simulationSnapshotKey` 与 `snapshotRowHash` 写入 canonical bytes/key/hash，worker 只可重读并核验该命名 v2 snapshot，v1 仍 literal unsupported。当前仅进行本地 TDD；完整 active loadout/effect/profile materialization、0030–0034 fresh/upgrade candidate 和既有页面验收前，ready-path 继续 literal blocked，route/UI binding、生产 migration/worker activation 和微信验收均未发生；Catalog/Manifest pointer、generation 35、Task 6C 和 async backflow 均不在本阶段。 | [Exact-first Catalog 设计](plans/2026-08-04-equipment-simulator-exact-first-catalog-design.md)、[持久化与运行链重排](plans/2026-08-04-equipment-simulator-exact-first-persistence-resequence.md)、[Task 5A 设计](plans/2026-08-07-equipment-simulator-exact-first-task5a-api-runtime.md)、[Task 5A requirement](../artifacts/releases/2026-08-07-equipment-simulator-exact-first-task5a/requirement.json)、[Task 3A evidence](../artifacts/releases/2026-08-04-equipment-simulator-exact-first/evidence.json)、[Task 3B evidence](../artifacts/releases/2026-08-06-equipment-simulator-exact-first-task3b/evidence.json)、[Task 4W evidence](../artifacts/releases/2026-08-06-equipment-simulator-exact-first-task4w/evidence.json) |
