...
- `正在推进`：测试账号快捷登录（2026-09-05 用户确认）。Mini/Web 使用 A/B 测试身份进入真实 Chat/SimC，保留正常会话与 owner 隔离；测试入口默认关闭且禁止生产启用。代码与隔离测试环境已就绪，真实登录/Chat/隔离/关闭 smoke 通过；待用户设置凭证及 Mini/Web/SimC 手工验收，尚未合入 main。实现和验收见 [任务计划](plans/2026-09-05-test-account-login.md)。
