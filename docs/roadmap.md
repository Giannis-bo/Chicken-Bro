...
| 正在纠正 | Exact-first 与三来源 Catalog 解耦 | Pure Canonical foundation 与 source change-control 已 final PASS/APPROVED；但原 Task 3 readiness 双审为 NOT_READY：旧 effect identity、authority dependency closure/canonical bytes、v2 store 顺序、worker role 与 job lifecycle 均不闭合。当前 forward replacement 固定为 3A authority bundle -> pure v2 -> 3B snapshot persistence -> worker/jobs；旧 Task 3 尚未启动。generation 35 未改写，也不声称真实 SimC runtime、持久化、API、候选部署、微信体验或 release 完成 | [Exact-first Catalog 设计](plans/2026-08-04-equipment-simulator-exact-first-catalog-design.md)、[Canonical Kernel 重设计](plans/2026-08-04-equipment-simulator-canonical-kernel-redesign.md)、[持久化与运行链重排](plans/2026-08-04-equipment-simulator-exact-first-persistence-resequence.md)、[原实施计划](plans/2026-08-04-equipment-simulator-exact-first-implementation.md) |
