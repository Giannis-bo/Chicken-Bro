...
| 待决策 | Exact-first 与三来源 Catalog 解耦 | Task 1 的 v1 兼容冻结和 Exact v2 输入合同已通过独立复审；旧 Task 2 已按五轮上限停止，Task 3 未启动。Canonical Kernel + sealed value objects 设计已确认，replacement plan 已就绪，下一步按五个独立 review gate 重建 Task 2；不得恢复第六轮局部补丁，不改写 generation 35 | [Exact-first Catalog 设计](plans/2026-08-04-equipment-simulator-exact-first-catalog-design.md)、[Canonical Kernel 重设计](plans/2026-08-04-equipment-simulator-canonical-kernel-redesign.md)、[replacement plan](plans/2026-08-04-equipment-simulator-canonical-kernel-implementation.md)、[后续实施计划](plans/2026-08-04-equipment-simulator-exact-first-implementation.md) |
