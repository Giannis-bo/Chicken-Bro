...
| P0 | 14 路由 UI 系统重建 | 共享 chrome、控件和素材槽先在微信三基线成立，再按固定批次传播；14 路由各完成一次真实运行态复核和核心交互验证；`builds_home` 收敛为职业选择器、四个平级工具入口和三条最近 SimC 任务预览 | [DESIGN.md](../DESIGN.md)、[current-ui](design/current-ui/README.md)、[当前计划](plans/ui-reconstruction.md)、[builds_home 设计](design/current-ui/routes/builds-home/product-design.md) |
...
| P1 | 构筑到模拟闭环稳定 | 天赋/装备模板可确定性保存、加载、转换、校验、提交和复盘；装备社区导入复用 80 个职业‑专精‑英雄天赋 winner 来源，并按职业‑专精提供两个合法真人模板；不可执行状态 fail closed | [装备模板投影计划](plans/2026-07-22-gear-template-projection.md)、[builds-architecture.md](builds-architecture.md)、[simulator-simc-end-to-end.md](simulator-simc-end-to-end.md) |
