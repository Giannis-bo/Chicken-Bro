...
| 正在推进 | Exact-first 与三来源 Catalog 解耦 | Pure Canonical foundation 与 source change-control 已 final PASS/APPROVED。Task 3A 当前为 `local_verified / candidate_pending`，runtime promotion truth 仍是 `candidate_rerun_required / evidence_promotion_blocked`：origin/main 已在 merge commit `1619815c` 无历史改写集成，主线 migrations 0026-0029 原样保留，Task 3A 顺延到唯一 0030；365 Task 3A、519 main backend、40 Node/Harness、TypeScript typecheck 与 25 Vitest 全部通过，独立 whole-branch CR 无未解决 code finding。第四次 `t3a260805120026` 真实通过旧 0026 链但已是 `runtime_passed_superseded_by_main_integration`；四个 run 的八库 immutable/non-reusable，必须以最终 clean packet head、第五个新 run-id 和两座新库重跑方可恢复 `runtime_verified`。获授权顺序仍为 3A -> 4P -> 4L -> 3B -> 4W，但本工作不授权 4P+；generation 35 未改写，runtimeConsumers 为空，无 production migration，也不声称真实 SimC runtime、API、微信体验、live 或 release 完成 | [Exact-first Catalog 设计](plans/2026-08-04-equipment-simulator-exact-first-catalog-design.md)、[Canonical Kernel 重设计](plans/2026-08-04-equipment-simulator-canonical-kernel-redesign.md)、[持久化与运行链重排](plans/2026-08-04-equipment-simulator-exact-first-persistence-resequence.md)、[Task 3A evidence](../artifacts/releases/2026-08-04-equipment-simulator-exact-first/evidence.json)、[原实施计划](plans/2026-08-04-equipment-simulator-exact-first-implementation.md) |
