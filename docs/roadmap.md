...
| 正在推进 | 炸鸡队长统一 ChatBot | Phase 1 将“队长”收敛为会话、新话题和存档三个入口；普通版本问答、个人模板与任务查询、允许来源查询仍走受控轻量 Tool；SimC 对比和 WCL 深度分析仍走 PostgreSQL 任务与一个通用 Worker；owner、版本、证据、新鲜度和失败降级可验证 | [后端架构](plans/2026-07-24-chickenbro-chatbot-design.md) · [Phase 1 聊天表面](plans/2026-08-01-chickenbro-chat-surface-design.md) · [实施计划](plans/2026-08-01-chickenbro-chat-surface-implementation.md) |
