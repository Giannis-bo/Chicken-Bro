...
- Web 会话切换修复：移除两处研究额度说明；A 会话回复过程中切走再返回时恢复等待、进度和流式内容，结束后可继续提问。本地交互回归、类型、lint 与 Web 构建通过；尚未发布或取得用户验收。[验证记录](../artifacts/verification/2026-09-11-chat-return-state/README.md)。
