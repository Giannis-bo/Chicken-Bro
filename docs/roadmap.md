...
| 待决策 | Exact-first 与三来源 Catalog 解耦 | Task 1 的 v1 兼容冻结和 Exact v2 输入合同已通过独立复审；Task 2 经 5 轮独立修复/复审仍剩 canonical slot 绑定与 Unicode control/newline 统一校验两个 Important，已按上限停止，Task 3 未启动。Canonical Kernel + sealed value objects 的重设计方向已确认，当前等待书面设计评审；不得以第 6 轮局部补丁继续，不改写 generation 35 | [Exact-first Catalog 设计](plans/2026-08-04-equipment-simulator-exact-first-catalog-design.md)、[Canonical Kernel 重设计](plans/2026-08-04-equipment-simulator-canonical-kernel-redesign.md)、[实施计划](plans/2026-08-04-equipment-simulator-exact-first-implementation.md) |
