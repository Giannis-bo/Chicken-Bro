...
| 正在推进 | Exact-first 与三来源 Catalog 解耦 | Pure Canonical foundation 与 source change-control 已 final PASS/APPROVED；Task 3A 首次真实 candidate 在 `0003` 之前失败：`0001` 已有 config-hash unique，而 `0003` 无条件新增同名 target constraint。当前严格为 `candidate_failed / migration_chain_blocked`；仅授权最小 fail-closed 0003 semantic-idempotence correction，failed run `t3a2608050955` 与两库不得复用，未生成 evidence/manifest。获授权顺序仍为 3A -> 4P -> 4L -> 3B -> 4W；generation 35 未改写，runtimeConsumers 为空，也不声称真实 SimC runtime、API、微信体验或 release 完成 | [Exact-first Catalog 设计](plans/2026-08-04-equipment-simulator-exact-first-catalog-design.md)、[Canonical Kernel 重设计](plans/2026-08-04-equipment-simulator-canonical-kernel-redesign.md)、[持久化与运行链重排](plans/2026-08-04-equipment-simulator-exact-first-persistence-resequence.md)、[原实施计划](plans/2026-08-04-equipment-simulator-exact-first-implementation.md) |
