...
| 正在推进 | 炸鸡队长 Codex 自主研究循环 | 用户已授权：Codex 自主理解研究目标、组合已发布的受控 ToolBox、根据观察迭代计划并判断结论；ToolBox 仅提供原子执行能力。职业、版本、场景、站点和固定问法不再成为回答路由。新增的是一个通用公开网页研究 Tool：Codex 可自己选择研究查询或安全公共 HTTPS 页面，后端只执行有界只读读取并返回引用快照；不为 Archon、Raider.IO、WCL 或某职业写固定 adapter/答案。通用 Tool 的每轮调用数由 manifest cost budget 声明，而不是“一次调用”的隐藏回答规则。可验证结论必须绑定本轮 observations 的真实引用；无 observation 时只返回 literal partial，流式内容在 claim 校验后才展示。候选 v5 Registry 通过服务环境显式 pin，不移动生产 v3 active pointer；真实微信验收待办，不代表生产投产 | [设计](plans/2026-08-04-chickenbro-agentic-research-design.md) · [实施计划](plans/2026-08-04-chickenbro-agentic-research-loop-implementation.md) · [通用网页 Tool 合同](plans/2026-08-04-chickenbro-generic-public-web-research-tool.md) |
