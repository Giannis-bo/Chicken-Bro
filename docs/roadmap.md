...
| 正在推进 | Exact-first 与三来源 Catalog 解耦 | Pure Canonical foundation 与 source change-control 已 final PASS/APPROVED。Task 3A 当前为 `runtime_verified / candidate_verified`：第八次 `t3a260805163536` 在 runtime commit `9f230f2e` / tree `89e7e5e` 通过 migration `0030` fresh/upgrade、attestation-v2、控制端与独立只读审计；verification commit `1714f376` / tree `7ba9ff18` 通过唯一 173-command full profile（Node 747、Python 2718，含 2 个预期 skip、Vitest 437）、Harness 与 candidate→verification 精确 owner test + active plan 两路径门禁。历史 PR CI run `30993690037` 的 29 个 Python 3.11 AST 序列化假失败已纠正，不属于 runtime candidate 失败。runtime/verification identities 已分离绑定；promotion window 仅允许 Task 3A 六份 evidence/status 文件，closure 仍 pending corrected PR CI、push、merge、合并后验证、归档与 scoped cleanup。第七次 `t3a260805160003` 继续以 `runtime_passed_unpromotable_current_truth_lifecycle_test_regression` 保留为历史，前七个 run 的十四库 immutable/non-reusable。获授权顺序仍为 3A -> 4P -> 4L -> 3B -> 4W，但本工作不授权 4P+；generation 35 未改写，runtimeConsumers 为空，无 production migration，也不声称真实 SimC runtime、API、微信体验、live 或 release 完成 | [Exact-first Catalog 设计](plans/2026-08-04-equipment-simulator-exact-first-catalog-design.md)、[Canonical Kernel 重设计](plans/2026-08-04-equipment-simulator-canonical-kernel-redesign.md)、[持久化与运行链重排](plans/2026-08-04-equipment-simulator-exact-first-persistence-resequence.md)、[Task 3A evidence](../artifacts/releases/2026-08-04-equipment-simulator-exact-first/evidence.json)、[原实施计划](plans/2026-08-04-equipment-simulator-exact-first-implementation.md) |
