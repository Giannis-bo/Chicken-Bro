...
| 下一步 | Exact-first 与三来源 Catalog 解耦 | 设计与四段实施计划已经形成但尚未授权执行；完整且可执行的 Exact 装备不因 Catalog 未收录被阻断，可换装 Catalog 保持为 SimC-ready Exact 的子集，动态发现只进入隔离 observation queue，经候选门禁和不可变 revision 发布 | [Exact-first Catalog 设计](plans/2026-08-04-equipment-simulator-exact-first-catalog-design.md)、[实施计划](plans/2026-08-04-equipment-simulator-exact-first-implementation.md) |
