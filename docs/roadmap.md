...
| 正在推进 | Exact-first 与三来源 Catalog 解耦 | Pure Canonical foundation 与 source change-control 已 final PASS/APPROVED。Task 3A 当前为 `implementation_allowed / candidate_pending`：第四次 `t3a260805120026` 保持 `runtime_passed_superseded_by_main_integration`；第五次 `t3a260805125812` 从 exact clean `a4c0fd04` / tree `aa0908eb` 真实通过 0030 fresh/upgrade PostgreSQL 链，其原始 attestation-v2 与十库继续 immutable/non-reusable；但 PR #114 full-profile run `30977816268` / job `92215539827` 的两个 direct-server import tests 证明其 exact source 不能从 `server/` 启动，因而历史状态为 `runtime_passed_invalidated_by_direct_runtime_import_regression`。当前工程真相恢复为 literal `candidate_rerun_required / evidence_promotion_blocked`，runtime 与 verification identity pending；controller full local profile 后必须第六次新 run-id/双库才可 promotion。获授权顺序仍为 3A -> 4P -> 4L -> 3B -> 4W，但本工作不授权 4P+；generation 35 未改写，runtimeConsumers 为空，无 production migration，也不声称真实 SimC runtime、API、微信体验、live 或 release 完成 | [Exact-first Catalog 设计](plans/2026-08-04-equipment-simulator-exact-first-catalog-design.md)、[Canonical Kernel 重设计](plans/2026-08-04-equipment-simulator-canonical-kernel-redesign.md)、[持久化与运行链重排](plans/2026-08-04-equipment-simulator-exact-first-persistence-resequence.md)、[Task 3A evidence](../artifacts/releases/2026-08-04-equipment-simulator-exact-first/evidence.json)、[原实施计划](plans/2026-08-04-equipment-simulator-exact-first-implementation.md) |
