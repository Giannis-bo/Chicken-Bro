...
| 正在推进 | Exact-first 与三来源 Catalog 解耦 | Pure Canonical foundation 与 source change-control 已 final PASS/APPROVED。Task 3A 当前为 `implementation_allowed / candidate_pending`。第七次 `t3a260805160003` 真实通过 migration `0030` PostgreSQL 与独立只读审计，但 exact-source full profile 因两份状态文档缺少 literal `0030`、current-truth test 硬编码 lifecycle stage 而失败，状态为 `runtime_passed_unpromotable_current_truth_lifecycle_test_regression`。当前工程真相仍为 literal `candidate_rerun_required / evidence_promotion_blocked`，runtime/verification/closure identities pending；七个 run 的十四库 immutable/non-reusable，第八次新 run-id/双库才可 promotion。获授权顺序仍为 3A -> 4P -> 4L -> 3B -> 4W，但本工作不授权 4P+；generation 35 未改写，runtimeConsumers 为空，无 production migration，也不声称真实 SimC runtime、API、微信体验、live 或 release 完成 | [Exact-first Catalog 设计](plans/2026-08-04-equipment-simulator-exact-first-catalog-design.md)、[Canonical Kernel 重设计](plans/2026-08-04-equipment-simulator-canonical-kernel-redesign.md)、[持久化与运行链重排](plans/2026-08-04-equipment-simulator-exact-first-persistence-resequence.md)、[Task 3A evidence](../artifacts/releases/2026-08-04-equipment-simulator-exact-first/evidence.json)、[原实施计划](plans/2026-08-04-equipment-simulator-exact-first-implementation.md) |
