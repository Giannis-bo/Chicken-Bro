...
| 正在推进 | Exact-first 与三来源 Catalog 解耦 | Pure Canonical foundation 与 source change-control 已 final PASS/APPROVED；原 Task 3 readiness 双审的阻塞已通过 forward replacement 纠正，两位独立第三轮 reviewer 均给出 Plan Spec PASS、Task 3A READY、0 findings。当前唯一获授权顺序为 3A authority bundle -> 4P pure v2（loadout effect fail closed）-> 4L loadout-effect authority -> 3B snapshot persistence -> 4W worker/jobs；现正冻结并实施 3A，双 fresh/upgrade PostgreSQL candidate 前保持 `candidate_pending`。generation 35 未改写，runtimeConsumers 为空，也不声称真实 SimC runtime、API、微信体验或 release 完成 | [Exact-first Catalog 设计](plans/2026-08-04-equipment-simulator-exact-first-catalog-design.md)、[Canonical Kernel 重设计](plans/2026-08-04-equipment-simulator-canonical-kernel-redesign.md)、[持久化与运行链重排](plans/2026-08-04-equipment-simulator-exact-first-persistence-resequence.md)、[原实施计划](plans/2026-08-04-equipment-simulator-exact-first-implementation.md) |
