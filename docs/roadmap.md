...
| 正在推进 | 炸鸡队长 Codex 自主研究循环 | 用户已授权：Codex 自主理解研究目标、组合受控 ToolBox、根据观察迭代并自然判断结论；ToolBox 仅提供原子执行能力。职业、版本、场景、站点和固定问法不再成为回答路由。当前候选改为原生 Codex Agent + MCP：同一 Agent 回合自行选择研究查询或安全公共 HTTPS 页面并调用通用公开网页 Tool，后端只执行有界只读读取、安全限制、会话和引用展示；不为 Archon、Raider.IO、WCL 或某职业写固定 adapter/答案。可验证结论绑定同一作业真实 MCP observations 的引用；不再以模型自报的 `claimRefs` 或 JSON 格式作为自然答案的发布门槛。候选 MCP profile 使用 gpt-5.6-luna，生产不变；真实微信验收待办，不代表生产投产 | [设计](plans/2026-08-04-chickenbro-agentic-research-design.md) · [实施计划](plans/2026-08-04-chickenbro-agentic-research-loop-implementation.md) · [通用网页 Tool 合同](plans/2026-08-04-chickenbro-generic-public-web-research-tool.md) |
