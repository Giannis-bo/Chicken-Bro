...
| 数据面 | `待重建` | 正式 legacy 与 v2 当前均使用名为 `wow_test` 的约 15GB 数据库；目标是迁移白名单数据到干净 `chickenbro_prod`，不是原地删表 |
| 云端空间 | `容量阻塞` | 2026-09-02 只读盘点为根盘 88%、仅约 8.1GB 可用，无法安全并存约 15GB 新旧生产库；四个无连接 gear evidence DB 合计约 21GB，但只能在独立恢复点和无引用证明后精确退役，否则必须先扩容 |
...
| 优先级 | 子项目 | 完成标准 |
| --- | --- | --- |
| 正在推进 | 控制面与清理清单 | 新规格通过书面复核；current truth、caller graph、keep/delete/migrate manifest 和分阶段实施计划形成 |
| 下一步 | 干净数据面与 Identity | `chickenbro_prod` 只含 identity/chat/simc/ops；小程序与 Web Session 解析到同一 `user_id` |
