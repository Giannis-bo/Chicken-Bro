...
| P0 | 14 路由 UI 系统重建 | 共享 chrome、控件和素材槽先在微信三基线成立，再按固定批次传播；14 路由各完成一次真实运行态复核和核心交互验证；`builds_home` 收敛为职业选择器、四个平级工具入口和三条最近 SimC 任务预览；`gear_detail` 的换装固定为“候选 → 后端返回的合法等级/变体配置 → 显式应用 → canonical Resolve”，不得点击候选即写入 | [DESIGN.md](../DESIGN.md)、[current-ui](design/current-ui/README.md)、[当前计划](plans/ui-reconstruction.md)、[builds_home 设计](design/current-ui/routes/builds-home/product-design.md) |
