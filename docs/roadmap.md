...
| 正在推进 | Exact-first 与三来源 Catalog 解耦 | 已获授权按四个 release slice 以 Subagent-Driven 方式实施；Task 1 的 v1 兼容冻结和 Exact v2 输入合同已通过独立复审，当前只推进 Task 2 的 Catalog-independent identity、效果支持三态与 authority envelope，任一 slice 门禁失败即停止，不改写 generation 35 | [Exact-first Catalog 设计](plans/2026-08-04-equipment-simulator-exact-first-catalog-design.md)、[实施计划](plans/2026-08-04-equipment-simulator-exact-first-implementation.md) |
