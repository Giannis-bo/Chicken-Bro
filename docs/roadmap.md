...
- `正在推进`：Web 输入区强化与等待反馈（2026-09-05 用户确认）。增强输入框层次，统一输入与正文的字体、字号和行高；回复位置显示动态等待提示，10 秒后显示耗时，正文到达后切换、失败时退出。实现与验证纳入 [测试 Web 任务](plans/2026-09-05-test-account-login.md)，用户体验验收待完成。
