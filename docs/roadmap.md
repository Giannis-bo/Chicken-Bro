...
| 待决策 | Exact-first 与三来源 Catalog 解耦 | Canonical 语义层的 NFC、bounded JSON、seal/reload、sealed consumers 与 v1/v2 identity regression 由独立行为测试覆盖；原 Task 5 静态 gate 在五轮复审后触发 Stop Gate，历史结论不包装为 clean。forward replacement Task 1 已以 `source_change_control_only` 完成四个 server declaration-only profile、一个 CLI bootstrap profile和 test-only registry；Task 1-2 final scoped reviews 均为 PASS/APPROVED、0 findings。该 gate 只证明源代码变更控制，不证明被登记 import 安全或任意 Python 语义。Fresh whole-branch review 仍 pending，原 Task 3 不得启动；generation 35 未改写，也不声称真实 SimC runtime、持久化、API、候选部署、微信体验或 release 完成 | [Exact-first Catalog 设计](plans/2026-08-04-equipment-simulator-exact-first-catalog-design.md)、[Canonical Kernel 重设计](plans/2026-08-04-equipment-simulator-canonical-kernel-redesign.md)、[owner change-control replacement](plans/2026-08-04-equipment-simulator-canonical-owner-change-control.md)、[后续实施计划](plans/2026-08-04-equipment-simulator-exact-first-implementation.md) |
