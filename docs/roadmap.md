...
| 正在推进 | Exact-first 与三来源 Catalog 解耦 | Pure Canonical foundation 与 source change-control 已 final PASS/APPROVED。Task 3A 的 `t3a2608050955`、`t3a260805111623` 分别因历史 `0003` 的重复约束与 `name[]`/`text[]` 比较缺陷在 `0026` 前失败；`t3a260805113656` 已在 commit `0e0743ca188efbd7cb818bfb8a210fd998379f4d` / tree `3fce81f57dbdc851a9438ae63ea051f666bf1051` 通过 fresh/upgrade runtime，但 active requirement 错为 `local_verified`，Harness 不允许证据晋升，且 candidate 后修正 requirement/owner contracts 会按既定边界使该运行失效。当前严格为 `candidate_rerun_required / evidence_promotion_blocked`；三个 run 的六库全部 immutable/non-reusable，未生成 evidence/manifest，必须在 `implementation_allowed` 的新 committed head 上使用第四个新 run-id 和两座新库重跑。获授权顺序仍为 3A -> 4P -> 4L -> 3B -> 4W；generation 35 未改写，runtimeConsumers 为空，也不声称真实 SimC runtime、API、微信体验或 release 完成 | [Exact-first Catalog 设计](plans/2026-08-04-equipment-simulator-exact-first-catalog-design.md)、[Canonical Kernel 重设计](plans/2026-08-04-equipment-simulator-canonical-kernel-redesign.md)、[持久化与运行链重排](plans/2026-08-04-equipment-simulator-exact-first-persistence-resequence.md)、[原实施计划](plans/2026-08-04-equipment-simulator-exact-first-implementation.md) |
