...
当前已完成清单与逐文件回读、用于职业/专精/天赋/装备事实图标的运行时媒体根是：

`https://api.chickenbro.cloud/wow-media/releases/2026-07-20-wow-icons-v1`

该版本包含 40 个专精所需的 3413 个文件，共 7,326,459 字节。2026-07-20 已从公开 HTTPS 根逐文件回读并核对 SHA-256。腾讯 CloudShell 当前因 MFA 过期无法把同一清单晋级到 COS，因此 `static.chickenbro.cloud` 的运行时媒体根仍不得使用；真机候选先使用已获微信后台批准的 `api.chickenbro.cloud` 不可变根。

小程序发布、候选构建和回滚都必须以上述两个已验证根为默认基线。除非一个新目录已经完成上传、逐文件回读校验和候选包验收，否则不得替换对应生产根。
