...
本文件保留整理前原文，不代表当前状态或新增授权。当前入口见[roadmap.md](roadmap.md)。历史链接保持原相对位置，按具体证据需要读取，不作为日常执行必读。
...
[通用研究证据与语义优化](plans/2026-09-11-research-generalization.md)：跨窗口证据、长对话约束、额度说明、模拟声明检查与固定评测；已授权实现、合入和发布，正在验证。
