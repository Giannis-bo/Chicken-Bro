...
本文件保留整理前原文，不代表当前状态或新增授权。当前入口见[badcase-workflow-operations.md](badcase-workflow-operations.md)。历史链接保持原相对位置，按具体证据需要读取，不作为日常执行必读。
...
每组方案与审查必须落实[通用修复与泛化验收](plans/2026-09-08-badcase-workflow.md#通用修复与泛化验收必须满足)：从失败机制提炼可复用的共同能力，优先扩展现有工具，通过参数表达场景差异；在现有组级机制、适用范围和审查说明中记录设计取舍及实际覆盖证据。按既定样本与预算验证同机制的不同使用场景，覆盖声明不得超过证据；不为抽象而新增工具、扩大范围或穷举验证。
...
执行泛化通用工具修复时，遵循[工作流的验证范围与停止条件](plans/2026-09-08-badcase-workflow.md#验证范围与停止条件)：不无限扩展验证场景，不陷入无尽穷举，适可而止。
