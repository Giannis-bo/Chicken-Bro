> 历史 Harness 存档；当前规则以 AGENTS.md、项目状态、验证矩阵及当前 harness.md 为准。
...
同时满足以下条件的 Light 任务走快车道：用户目标明确且范围有界；变更是文案、样式、展示收敛、明确 bug 或对应测试补充；不改变 API、数据、owner、运行时、部署或用户承诺语义。

- Agent 只需先给出“改什么、风险边界、定向验证”三项简短说明。只有用户可见行为存在真实歧义时才问一个澄清问题；不得为了凑流程再要求设计确认。
- Agent 根据任务独立性、风险和当前工作区状态自行选择是否使用子代理、worktree 或普通分支；不得把这些内部执行拓扑或常规合并方式交给用户点选。
- 快车道不创建设计文档、实施计划或仅为流程而存在的文档提交；实际改动、定向测试和一次本地 CR 足够时，应直接推进。
- 一旦发现 API、数据、owner、运行时、部署语义或用户承诺改变，立即退出快车道，按 Standard 或 Strict 重新分级。

### User Acceptance Closure

若任务需要用户手工验证，Agent 在完成实现和定向验证后等待明确的验收语句，例如“我已测试通过”“可以收尾”或“合入吧”。中途单独的“OK”只表示当前讨论/步骤获准，不是合并授权。

收到明确验收后，Agent 不再弹出交付方式菜单，而是按以下顺序自动收尾：
