...
| 专门的“炸鸡队长”版本问题分析 Agent | 当前对话，2026-06-18 用户要求新增到想法池；2026-06-18 用户确认补充对话边界、同类横评 v1 和高端日志学习方向 | 当前资讯、PVE、职业专精、SimC、WebSim 和日志复盘能力仍分散；如果不约束对话范围和最终输出，Agent 容易变成泛聊天，而不是帮助正式服当前版本玩家判断自己表现和下一步提升路径的核心工具。 | 将“分析当前版本各种问题并给玩家建议”沉淀为小程序最核心的能力：只面向魔兽世界正式服当前版本，版本刚迭代时允许短暂宽容窗口但必须标注数据新鲜度和不确定性；非 WoW、非正式服当前版本、闲聊或无关话题直接回避并引导回角色、日志、构筑、装备、手法或版本问题；v1 以“同类横评”为核心结论，按同职业、同专精、同职责、同分段、同装等、同场景给出排名/分位、DPS/HPS/生存表现和差距判断，并输出天赋、手法、装备、属性、技能时间线等按优先级排序的调优建议；后台知识来源与下一条“顶级 M+ / 团本日志与手法知识库自动沉淀”配套。 | 需要定义“炸鸡队长”的人格边界、拒答策略、正式服版本窗口、同类玩家筛选规则、报告 schema、评测集和失败回退；所有结论必须区分官方数据、日志样本、SimC 结果、WebSim 输入、本地知识库证据和 LLM 推理；缺少 WCL、`/simc`、角色/装等/场景等关键输入时只能给通用建议，不能伪造个人排名或已验证事实。 | 产品定位命名 / WCL / 日志复盘链路 / LLM 报告 schema 化 / 数据与模拟健康哨兵 / 深度 Codex Agent 工作流（需后续拆出专门里程碑） | 候选 |
