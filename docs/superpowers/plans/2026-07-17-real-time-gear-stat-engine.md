...
Use a context key of `"{classKey}:{specKey}:{level}:{raceKey}"`. The synthetic schema initially defines `primaryKey`, `baseAttributes`, ordered `stableModifiers`, `resources`, and `secondaryRules`; its linear fixture uses `inputKey`, `outputKey`, `label`, `basePercent`, `ratingPerPercent`, `precision`, `sourceRefs`, and `displayUnit`. Before any production context is promoted, extend this to a declarative `ratingTransform` that can represent the verified level scaling, diminishing curve and ordered additive/multiplicative effects for that output. `ratingPerPercent` remains fixture-only compatibility and cannot be used to approximate a production curve. Store source references and golden sample IDs in the rulebook, but strip only non-public implementation notes from the payload; do not strip revisions or sources.
...
Candidate preparation includes `mage-frost-armory-2026-07-17t041853z` (historical 404), `mage-frost-armory-2026-07-17t091243z` (current official Frost Dwarf capture, including the opaque active Frost Spellslinger loadout identity) and `mage-arcane-armory-2026-07-17t041853z` (current Arcane Night Elf capture, including its opaque active Arcane Spellslinger loadout identity), all with explicit `candidate` status. The failing-first registry test proves that a `candidate` cannot satisfy `goldenSampleIds` validation. This step stays open until at least one identity- and calculator-input-complete sample can run the field-by-field golden loop: Frost and Arcane now have exact official identity/equipment/panel/loadout snapshots and one shared curve capability is proven (`21025`, Frost avoidance `470 → 12.217245%`), but each still lacks canonical variant/static-stat, enhancement-value, stable-versus-conditional talent-effect mapping, operation ordering and full field-level formula evidence.
