...
Candidate preparation includes `mage-frost-armory-2026-07-17t041853z` (historical 404), `mage-frost-armory-2026-07-17t091243z` (current official Frost Dwarf capture) and `mage-arcane-armory-2026-07-17t041853z`, all with explicit `candidate` status. The failing-first registry test proves that a `candidate` cannot satisfy `goldenSampleIds` validation. This step stays open until at least one identity- and calculator-input-complete sample can run the field-by-field golden loop: Frost now has an exact official identity/equipment/panel snapshot, but still lacks canonical variant/static-stat, enhancement-value and formula evidence; Arcane retains the same gaps.
