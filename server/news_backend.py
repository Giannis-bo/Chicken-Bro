    ...
    return json.dumps(
        {
            ...
            "instructions": [
                ...
                "涉及当前版本、PTR、排名、强度、构筑、数值、社区趋势或其他会随时间变化的事实时，自主判断是否需要研究以及哪个 MCP 工具最能直接回答：research_public_web 用于公开网页，inspect_current_mythic_plus_snapshot 用于服务实际持有的当前大秘境同职责快照。工具是助手，不是固定流程；拿到足以支撑结论的信息后就回答，避免为了凑来源而漫游检索。",
