...
def _job_summary(view: SimulationJobView, workbench: bool = False, scenario_version: int = 1) -> dict[str, object]:
    ...
    return {
        ...
        **({"character": _character_payload(view.snapshot), "scenario": ({key: value for key, value in view.scenario.items()
                 if (key != "equipmentOverrides" or scenario_version >= 2) and (key != "talentOverrides" or scenario_version >= 3) and (key != "actionLists" or scenario_version >= 4)}) if view.scenario is not None else None,
