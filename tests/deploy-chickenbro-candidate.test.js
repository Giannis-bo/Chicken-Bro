...
test('candidate deploy is content-addressed, candidate-only, reversible and exact-smoked', () => {
  ...
  assert.match(script, /WOW_CHICKENBRO_ACCEPTANCE_SOURCE_URL/)
  assert.match(script, /acceptance source URL is required/)
...
test('candidate deploy records identities without serializing credentials', () => {
  ...
  assert.match(script, /--source-url "\$\{ACCEPTANCE_SOURCE_URL\}"/)
