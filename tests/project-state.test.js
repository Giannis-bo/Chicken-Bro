...
test('project-state is the single machine-readable current truth entry', () => {
  ...
  assert.equal(
    catalogMigrationPhase0?.auditReportId,
    'catalog-migration-audit:sha256:1f211147e19479a67c84641904222b9a4612ede8caafa6c0d2e328898c9341d2',
  )
  ...
  assert.deepEqual(catalogMigrationPhase0?.catalogMapping, {
    itemTotal: 1486,
    mappedItemCount: 1310,
    excludedNonCatalogItemCount: 176,
    variantTotal: 50528,
    legacyBrowseVariantTotal: 1674,
    canonicalBrowseVariantTotal: 1309,
    mappedBrowseVariantCount: 1309,
    craftedEnhancementSelectionRowCount: 438,
    collapsedCraftedVariantRowCount: 365,
    progressionCounts: {
      upgrade_track: {
        legacyRowCount: 1113,
        canonicalCandidateCount: 1113,
        mappedCandidateCount: 1113,
      },
      crafted_quality: {
        legacyRowCount: 324,
      ...
      ascendant: {
        legacyRowCount: 237,
        canonicalCandidateCount: 142,
        mappedCandidateCount: 142,
    ...
    problemCounts: {},
    excludedExactInstanceCount: 48555,
    excludedPlaceholderVariantCount: 288,
  ...
  assert.equal(catalogMigrationDecision.problems[0].facts.canonicalBrowseVariantTotal, 1313)
  assert.equal(catalogMigrationDecision.problems[0].facts.mappedBrowseVariantCount, 1265)
  ...
  assert.equal(gearReleaseTrain.status, 'active_manifest_phase0_verified')
  assert.equal(gearReleaseTrain.pointerGeneration, 32)
  assert.equal(gearReleaseTrain.activeManifestRevision, 'season-manifest:sha256:b79e6226c7dc406c2e729cf01cf6d86f4944b760b8e8cc185dcd4347032fedb6')
  assert.equal(gearReleaseTrain.rollbackManifestRevision, 'season-manifest:sha256:ab1cdff36be524d9f6f499732e0ffca88e4eea39bb6013d735a33a5a01a3b88a')
...
test('real WeChat interaction verification cannot wait forever inside one route', () => {
  ...
  assert.match(verifier, /writeBoundedJsonAtomic/)
...
test('visual review capture stays explicit, cached and out of the main session', () => {
  ...
  assert.match(capture, /inspectRenderer/)
  assert.match(capture, /renderer shell missing/)
  assert.match(capture, /renderer regions missing/)
  ...
  assert.match(capture, /readBoundedFile\(artifactPath, maxCaptureBytes/)
  assert.match(capture, /readBoundedJson\(manifestPath/)
  assert.match(capture, /writeBoundedJsonAtomic\(manifestPath/)
  ...
  assert.match(capture, /wechat-ui-review-cache-v4/)
  assert.match(capture, /reused_existing_wechat_devtools_process/)
  assert.match(capture, /mini_program_relaunch/)
...
test('cached visual review requires explicit immutable promotion', () => {
  ...
  assert.match(promotion, /readBoundedFile\(capture\.artifactPath, maxCaptureBytes/)
  assert.match(promotion, /readBoundedJson\(manifestPath/)
  assert.match(promotion, /manifest\.captures\.length > 14/)
  assert.match(promotion, /wechat-ui-runtime-promotion-v4/)
...
test('selected control verification is bounded and cannot launch DevTools', () => {
  ...
  const { expectedBoundarySequence } = require('../scripts/verify-ui-selected-states.js')
  ...
  assert.ok(contract.groups.every((group) => ['contiguous', 'isolated'].includes(group.boundaryMode)))
  assert.ok(contract.groups.filter((group) => group.boundaryMode === 'contiguous').length >= 5)
  ...
  assert.equal(contract.materialOwnership.controlMaterialOwner, 'css')
  assert.equal(contract.materialOwnership.boundaryAttribute, 'data-leading-boundary')
  assert.deepEqual(expectedBoundarySequence(['true', 'false', 'false']), ['active', 'suppressed', 'inactive'])
  ...
  assert.match(verifier, /materialOwnerMismatches/)
  assert.match(verifier, /nestedMaterialRenderCount/)
  assert.match(verifier, /SELECTED_STATE_DETAIL_PATH/)
  assert.match(verifier, /wechat-selected-control-detail-v3/)
  assert.match(verifier, /expectedLeadingBoundaries/)
  assert.match(verifier, /boundaryMismatches/)
  assert.match(verifier, /maximumControlsPerGroup = 32/)
  assert.match(verifier, /queryCapExceeded/)
...
test('selected control evidence promotion is exact, immutable and content addressed', () => {
  ...
  assert.match(promotion, /visualMaterialDistinct === true/)
  ...
  assert.match(verifier, /border-left-color/)
  assert.match(verifier, /box-shadow/)
})
...
test('native buttons and selected segments have exclusive geometry owners', () => {
  ...
  assert.match(audit, /route_geometry_has_no_region_overflow_exemptions/)
  assert.match(audit, /horizontal_scroll_exemptions_never_bypass_native_button_width/)
  assert.match(audit, /initial_safe_area_button_contract_is_presence_complete/)
...
test('raster integration metadata cannot drift from registry and source bindings', () => {
  ...
  const buildsHome = readJson('packages/design-system/assets/raster/builds-home-v1/manifest.json')
  ...
  const newsHome = readJson('packages/design-system/assets/raster/news-home-v1/manifest.json')
  assert.equal(buildIntel.registeredInPackagesAssetsManifest, true)
  assert.equal(buildIntel.wiredIntoRuntimeCode, true)
  assert.equal(buildIntel.runtimeBindingStatus, 'complete_3_of_3')
  assert.equal(buildsHome.registeredInPackagesAssetsManifest, true)
  assert.equal(buildsHome.wiredIntoRuntimeCode, true)
  assert.equal(buildsHome.productionPromoted, false)
  assert.equal(buildsHome.runtimeBindingStatus, 'complete_23_of_23')
  assert.equal(newsDetail.wiredIntoRuntimeCode, true)
  assert.equal(newsDetail.runtimeBindingStatus, 'complete_3_of_3')
  const newsList = readJson('packages/design-system/assets/raster/news-list-v1/manifest.json')
  assert.equal(newsList.runtimeBindingStatus, 'complete_2_of_2')
  ...
  assert.equal(sharedChrome.runtimeBindingStatus, 'partial_1_of_5')
...
test('route geometry verification covers all routes without launching DevTools', () => {
  ...
  assert.ok(contract.routes.every((route) => new Set(route.requiredRegionIds).size === route.requiredRegionIds.length))
  ...
  assert.match(verifier, /missing-semantic-region/)
  assert.match(verifier, /unexpected-semantic-region/)
  assert.match(verifier, /requiredRegionIds/)
  assert.match(verifier, /missingRequiredRegionCount/)
  assert.match(verifier, /unexpectedRegionCount/)
  assert.match(verifier, /route\.minimumRegions \?\? 1/)
  assert.match(verifier, /anonymous-region-id/)
  assert.match(verifier, /duplicate-semantic-region/)
  assert.match(verifier, /duplicateRegionCount/)
  assert.match(verifier, /native-button-horizontal/)
  assert.match(verifier, /native-button-width/)
  ...
  assert.match(verifier, /anonymous-native-button/)
  assert.match(verifier, /native-button-cell-overflow/)
  assert.match(verifier, /data-control-roles/)
  assert.equal(contract.routes.some((route) => (route.allowedVerticalOverflowButtonRoles ?? []).length > 0), false)
  assert.equal(contract.routes.some((route) => (route.allowedVerticalOverflowRegions ?? []).length > 0), false)
  ...
  assert.match(verifier, /maxInitialSafeAreaRegionBottom/)
  assert.match(verifier, /tab-root-scroll-viewport-overlap/)
  assert.match(verifier, /for \(const role of initialSafeAreaButtonRoles\)/)
  const tasks = contract.routes.find((route) => route.route === 'tasks_list')
  assert.deepEqual(tasks.initialSafeAreaRegionIds, ['bottom_actions'])
  ...
  assert.match(verifier, /queriedRegions\.slice\(0, queryCaps\.regions\)/)
  assert.match(verifier, /connectMiniProgram/)
  assert.doesNotMatch(verifier, /WECHAT_AUTOMATOR_LAUNCH/)
  ...
  assert.deepEqual(simcSubmit.initialSafeAreaRegionIds, ['submission_footer'])
  const buildIntel = geometryContract.routes.find((route) => route.route === 'build_intel')
  assert.deepEqual(buildIntel.initialSafeAreaRegionIds, ['reference_disclaimer'])
  assert.deepEqual(taskDetail.initialSafeAreaRegionIds, ['exception_state'])
...
test('route geometry evidence promotion is exact, safe-area aware and immutable', () => {
  ...
  assert.match(promotion, /capsuleBounds/)
  assert.match(promotion, /capsuleFitsWindow/)
...
test('runtime review regions cannot fall back to positional identities', () => {
  ...
  assert.match(audit, /route_stage_target_counts_match_component_contracts/)
  assert.match(audit, /valuesForKey\(componentContractsByRoute\.get\(route\.route\), 'stableRegionCount'\)/)
})
...
test('runtime asset-slot review is contract-mapped and bounded', () => {
  ...
  assert.match(verifier, /async function main\(\) \{\s+const routes = selectedRoutes\(\)\s+validateContract\(\)/)
  assert.match(verifier, /queryCaps = Object\.freeze/)
  assert.match(verifier, /asset element query cap exceeded/)
  ...
  assert.match(verifier, /wechat-runtime-asset-slot-review-v3/)
  assert.match(verifier, /invalidMaterialOwnerElements/)
...
test('raster runtime assets are byte and hash verified without image payloads', () => {
  ...
  assert.match(verifier, /maximumRuntimeAssetBytes = 8 \* 1024 \* 1024/)
  assert.match(verifier, /Buffer\.allocUnsafe\(64 \* 1024\)/)
  assert.match(verifier, /runtime asset changed while hashing/)
  assert.match(verifier, /readBoundedFile\(path\.join\(rasterRoot, entry\.name, 'manifest\.json'\), maximumManifestBytes/)
  assert.doesNotMatch(verifier, /readFileSync\(record\.path\)/)
  assert.match(verifier, /failures\.slice\(0, 10\)/)
...
test('SimC regions scale inside the shell content viewport', () => {
  ...
  assert.match(audit, /region\.height <= 0/)
  assert.match(audit, /region\.width <= 0/)
  assert.match(audit, /all_anchored_route_regions_have_numeric_minimum_viewport_geometry/)
  ...
  assert.match(audit, /fixed_wrapper_widths_fit_narrow_content_boxes/)
  assert.match(audit, /negativeHorizontalWrapperMargins/)
  assert.match(audit, /largeHorizontalPositionEscapes/)
  assert.match(audit, /percentage_width_expansion_is_owned_by_an_explicit_production_asset_shell/)
  assert.match(audit, /every_data_selected_publisher_has_a_stable_contracted_material_owner/)
  ...
  assert.match(audit, /talent_simulator_geometry_has_one_component_style_owner/)
  assert.match(audit, /task_routes_have_no_parallel_reconstruction_geometry_owner/)
  assert.match(audit, /simulator_and_chickenbro_geometry_have_no_parallel_reconstruction_owner/)
  assert.match(audit, /profile_geometry_has_no_parallel_reconstruction_owner/)
  ...
  assert.match(audit, /all_online_wechat_verifiers_close_route_batches_before_connecting/)
  ...
  assert.match(audit, /invalidRouteStageRegionStyleBindings/)
  assert.match(audit, /all_route_region_style_references_resolve/)
  assert.match(audit, /invalidRouteRegionStyleReferences/)
  assert.match(audit, /all_literal_route_css_module_references_resolve/)
  assert.match(audit, /missingLiteralRouteCssModuleReferences/)
  assert.match(audit, /selected_material_publishers_bind_active_style_owners/)
  ...
  assert.match(audit, /all_asset_owned_controls_remove_the_css_plate/)
  assert.match(audit, /fullFrameAssetOwnedControls/)
  assert.match(audit, /offline_review_index_is_bounded_image_free_and_cache_verified/)
  assert.match(audit, /reviewIndexBuilder/)
  assert.match(audit, /ui_review_cache_artifacts_stay_inside_the_manifest_directory/)
  assert.match(audit, /reviewCachePromotion/)
