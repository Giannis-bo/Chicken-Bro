...
class AdminPostgresTest(unittest.TestCase):
    def test_aggregation_timezone_population_distinct_feedback_provenance_and_empty(self):
        ...
        with connect() as conn:
            ...
            # A build-only user must be active in POE2 without entering WoW activity.
            build_only=uuid4()
        ...
        self.assertEqual(data['users'],{'total':3,'new':2,'active':2})
        ...
        self.assertEqual(data['daily'],[{'date':'2026-09-09','newUsers':2,'activeUsers':2,'questions':4,'simulations':5,'builds':0}])
        ...
        self.assertEqual(poe_data['chat']['total'],1)
        self.assertEqual(poe_data['users']['active'],2)
        ...
        self.assertEqual(poe_data['daily'][0]['questions'],1)
