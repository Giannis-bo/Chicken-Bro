...
test('backend owner map defines the backend hotspot ownership contract', () => {
  ...
  assert.ok(
    gearCatalogReadModelSelectors.owns.includes('websim loot response envelope read-model assembly'),
    'gear catalog read-model selectors should own websim loot response envelope assembly after this Phase 4 split'
  )
  assert.ok(
    gearCatalogReadModelSelectors.owns.includes('websim talent authority read-model assembly'),
    'gear catalog read-model selectors should own websim talent authority assembly after this Phase 4 split'
  )
  assert.ok(
    gearCatalogReadModelSelectors.owns.includes('websim community talent template row/list read-model assembly'),
  ...
  assert.ok(
    gearCatalogReadModelSelectors.owns.includes('websim profile preset read-model assembly'),
    'gear catalog read-model selectors should own websim profile preset assembly after this Phase 4 split'
  )
  assert.ok(
    gearCatalogReadModelSelectors.owns.includes('websim default selection read-model assembly'),
  ...
  assert.ok(
    gearCatalogReadModelSelectors.owns.includes('websim talent import template read-model assembly'),
    'gear catalog read-model selectors should own websim talent import template assembly after this Phase 4 split'
  )
  assert.ok(
  ...
  assert.ok(
    gearCatalogReadModelSelectors.characterization.some((entry) => entry.includes('test_build_websim_loot_read_model_wraps_items_instances_and_season')),
    'gear catalog read-model selectors should point at the websim loot envelope read-model characterization test'
  )
  assert.ok(
    gearCatalogReadModelSelectors.characterization.some((entry) => entry.includes('test_build_websim_talent_authority_read_model_preserves_runtime_and_official_state')),
    'gear catalog read-model selectors should point at the websim talent authority read-model characterization test'
  )
  assert.ok(
  ...
  assert.ok(
    gearCatalogReadModelSelectors.characterization.some((entry) => entry.includes('test_build_websim_profile_presets_read_model_maps_rows')),
    'gear catalog read-model selectors should point at the websim profile preset read-model characterization test'
  )
  assert.ok(
  ...
  assert.ok(
    gearCatalogReadModelSelectors.characterization.some((entry) => entry.includes('test_build_websim_talent_import_template_read_model_maps_row')),
    'gear catalog read-model selectors should point at the websim talent import template read-model characterization test'
  )
  assert.ok(
  ...
  const cachePayloadReadModelSelectors = pgOwners.get('cache_payload_read_model_selectors')
  assert.ok(cachePayloadReadModelSelectors)
  assert.equal(cachePayloadReadModelSelectors.status, 'extracted_selector')
  assert.equal(cachePayloadReadModelSelectors.extractedModule, 'server/pg_cache_read_model_selectors.py')
  assert.ok(
    cachePayloadReadModelSelectors.owns.includes('Raider.IO cache row read-model assembly'),
