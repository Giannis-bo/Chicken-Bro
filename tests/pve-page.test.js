...
test('pve home and detail pages use a dark competitive dungeon palette', () => {
  ...
  assert.match(homeCss, /\.pve-hero[\s\S]*#17120d/i)
  assert.match(homeCss, /\.pve-hero[\s\S]*rgba\(167,\s*26,\s*25,\s*0\.18\)/i)
  assert.match(homeCss, /\.module-footer[\s\S]*#f8b700/i)
  assert.match(detailCss, /\.detail-hero[\s\S]*#17120d/i)
  assert.match(detailCss, /\.detail-hero[\s\S]*rgba\(167,\s*26,\s*25,\s*0\.18\)/i)
  assert.match(detailCss, /\.record-value[\s\S]*#f8b700/i)
  assert.match(homeCss, /\.zone-item[\s\S]*border-radius:\s*12rpx;/)
