...
export default function ChatFeedbackConfirm({ resolved, onConfirm, onCancel }: Props) {
  ...
  return <dialog ref={dialog} className={styles['dialog']} aria-labelledby={titleId} aria-describedby={descriptionId}
    ...
    <div className={styles['actions']}>
      <button ref={cancel} type="button" className={styles['actionButton']} onClick={onCancel}>取消</button>
      <button type="button" className={`${styles['actionButton']} ${styles['confirm']}`} onClick={onConfirm}>确认</button>
    </div>
  </dialog>
}
