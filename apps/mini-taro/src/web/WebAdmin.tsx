    ...
    <div className={styles['content']}>
      ...
      {access?.isAdmin ? <>
        ...
        {data ? <>
          ...
          <section className={styles['panel']}><div className={styles['panelTitle']}><div><h2>使用趋势</h2><p>每日统计，缺少记录的日期显示为 0</p></div><div className={styles['presets']}>{([['activeUsers','活跃用户'],['questions','提问数'],['simulations','模拟任务']] as const).map(([key,label])=><button className={styles['control']} key={key} aria-pressed={series===key} onClick={()=>setSeries(key)}>{label}</button>)}</div></div>
