...
import type { AdminAccess, AdminOverview } from '@wow-mini/domain'
...
export default function WebAdmin({auth,accountLabel,onLogout,client=wowApi.admin}: {auth: Auth; accountLabel: string; onLogout: () => void; client?: AdminClient}) {
  ...
  const [series,setSeries]=useState<'activeUsers'|'questions'|'simulations'>('activeUsers')
  ...
  useEffect(()=>{
    ...
    const load=async()=>{
      ...
      try {
        ...
        const result=await client.overview(auth,...range)
        if(!alive || intent!==sequence.current) return
        if(result.fromFallback || !result.payload) {
  ...
  return <main className={styles['admin']}>
    <header className={styles['header']}><a className={styles['brand']} href={webViewHref('chat')}>炸鸡队长来啦<span>运营后台</span></a><div className={styles['account']}><span>{accountLabel}</span><a href={webViewHref('chat')}>返回网站</a><button onClick={onLogout}>退出登录</button></div></header>
    <div className={styles['content']}>
      <div className={styles['heading']}><div><p className={styles['eyebrow']}>CHICKENBRO / OPERATIONS</p><h1>运营概览</h1><p>了解用户使用情况、回答质量与模拟表现。</p></div><span className={styles['badge']}>只读 · 管理员专属</span></div>
      {error ? <div className={styles['notice']} role="alert">{error}<button onClick={()=>setRefresh(v=>v+1)}>重试</button></div> : null}
      ...
      {access?.isAdmin ? <>
        <section className={styles['filters']} aria-label="统计日期"><div className={styles['presets']}>{[[1,'今天'],[7,'近 7 天'],[30,'近 30 天']].map(([days,label])=><button key={days} aria-pressed={range.join()===presetDates(Number(days)).join()} onClick={()=>selectRange(Number(days))}>{label}</button>)}</div><div className={styles['dates']}><label>开始日期<input aria-label="开始日期" type="date" value={draft[0]} max={beijingDate()} onChange={e=>setDraft([e.target.value,draft[1]])} /></label><span>—</span><label>结束日期<input aria-label="结束日期" type="date" value={draft[1]} max={beijingDate()} onChange={e=>setDraft([draft[0],e.target.value])} /></label><button onClick={apply}>查询</button><button disabled={busy} onClick={()=>setRefresh(v=>v+1)}>刷新</button></div></section>
        {data ? <>
          <div className={styles['snapshot']}><span>{data.start} — {data.end} · 北京时间</span><span>更新于 {new Date(data.generatedAt).toLocaleString('zh-CN',{timeZone:'Asia/Shanghai',hour12:false})}</span></div>
          <section className={styles['stats']} aria-label="核心指标"><Stat label="累计用户" value={number(data.users.total)} hint="截至所选结束日的 QQ 用户"/><Stat label="新增用户" value={number(data.users.new)} hint="所选期间首次 QQ 登录"/><Stat label="活跃用户" value={number(data.users.active)} hint="提问或提交模拟的去重用户"/><Stat label="提问数" value={number(data.chat.total)} hint="所选期间发起的回复"/><Stat label="模拟任务" value={number(data.simc.total)} hint="含网站与对话工具提交"/></section>
          <section className={styles['panel']}><div className={styles['panelTitle']}><div><h2>使用趋势</h2><p>每日统计，缺少记录的日期显示为 0</p></div><div className={styles['presets']}>{([['activeUsers','活跃用户'],['questions','提问数'],['simulations','模拟任务']] as const).map(([key,label])=><button key={key} aria-pressed={series===key} onClick={()=>setSeries(key)}>{label}</button>)}</div></div>
            ...
            <details className={styles['details']}><summary>查看每日明细</summary><div className={styles['tableWrap']}><table><thead><tr><th>日期</th><th>新增用户</th><th>活跃用户</th><th>提问数</th><th>模拟任务</th></tr></thead><tbody>{data.daily.map(d=><tr key={d.date}><td>{d.date}</td><td>{d.newUsers}</td><td>{d.activeUsers}</td><td>{d.questions}</td><td>{d.simulations}</td></tr>)}</tbody></table></div></details>
          </section>
          <div className={styles['grid']}>
            <section className={styles['panel']}><h2>鸡哥对话</h2><div className={styles['featureStats']}><Stat label="回复成功率" value={formatRate(data.chat.successRate)}/><Stat label="反馈解决率" value={formatRate(data.chat.resolutionRate)}/></div><dl><Metric label="成功 / 失败 / 进行中" value={`${data.chat.succeeded} / ${data.chat.failed} / ${data.chat.running}`}/><Metric label="已解决 / 未解决" value={`${data.chat.resolved} / ${data.chat.unresolved}`}/><Metric label="反馈率" value={formatRate(data.chat.feedbackRate)}/><Metric label="平均 / P95 回复耗时" value={`${duration(data.chat.avgSeconds)} / ${duration(data.chat.p95Seconds)}`}/></dl><p className={styles['footnote']}>成功率以已结束回复为分母；反馈率以成功回复为分母；解决率仅代表已提交反馈的回复。</p></section>
            <section className={styles['panel']}><h2>SimC 模拟</h2><div className={styles['featureStats']}><Stat label="模拟成功率" value={formatRate(data.simc.successRate)}/><Stat label="排队 / 运行中" value={`${data.simc.queued} / ${data.simc.running}`}/></div><dl><Metric label="成功 / 失败 / 取消" value={`${data.simc.succeeded} / ${data.simc.failed} / ${data.simc.cancelled}`}/><Metric label="结果异常" value={number(data.simc.invalidResults)}/><Metric label="平均 / P95 完成耗时" value={`${duration(data.simc.avgSeconds)} / ${duration(data.simc.p95Seconds)}`}/></dl><p className={styles['footnote']}>成功要求有效指标和来源记录。成功率分母包含成功、失败及结果异常；耗时含排队，仅统计已结束任务。</p></section>
          </div>
          <section className={styles['panel']}><h2>职业与专精分布</h2><p className={styles['footnote']}>按模拟任务来源角色统计，每个任务计一次。</p>{data.specializations.length ? <div className={styles['tableWrap']}><table><thead><tr><th>职业</th><th>专精</th><th>任务数</th><th>占比</th></tr></thead><tbody>{data.specializations.map(row=><tr key={`${row.class}/${row.spec}`}><td>{localName(row.class,classNames)}</td><td>{localName(row.spec,specNames)}</td><td>{number(row.count)}</td><td>{formatRate(row.count/data.simc.total)}</td></tr>)}</tbody></table></div> : <p className={styles['empty']}>所选期间暂无模拟任务</p>}</section>
          <footer className={styles['method']}><strong>统计口径</strong><p>仅统计当前 QQ 应用关联用户，排除固定测试账号、已知模拟验收身份及无 QQ 身份的历史记录。活跃用户按期间去重；各业务按发起日期归入所选期间，展示查询时的最新状态和反馈。归档会话仍计入。没有样本的比例或耗时显示「—」。页面访问量、Token 和费用尚未纳入。</p></footer>
        </> : null}
      </> : null}
    </div>
  </main>
