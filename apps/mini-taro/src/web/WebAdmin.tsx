    ...
    <div className={styles['content']}>
      ...
      {access?.isAdmin ? <>
        ...
        {data && tasks ? <>
          ...
          <section className={styles['stats']} aria-label="核心指标"><Stat label="累计用户" value={number(data.users.total)} hint="当前游戏 · 截至结束日提问过的用户"/><Stat label="新增用户" value={number(data.users.new)} hint="当前游戏 · 期间首次提问的用户"/><Stat label="活跃用户" value={number(data.users.active)} hint="当前游戏 · 期间提问用户去重"/><Stat label="提问数" value={number(data.chat.total)} hint="所选期间发起的回复"/><Stat label={taskLabel} value={number(tasks.total)} hint="含网站与对话工具提交"/></section>
          ...
          <footer className={styles['method']}><strong>统计口径</strong><p>仅统计当前 QQ 应用关联用户，排除固定测试账号、已知模拟验收身份及无 QQ 身份的历史记录。用户按实际提问所属游戏分别去重：两款游戏都提问过的用户，两边各计一次。累计用户截至所选结束日；新增用户按该游戏首次提问日期统计；活跃用户仅统计期间提问。仅登录、空会话、构筑导入或计算不计入对话用户数；各业务按发起日期归入所选期间，展示查询时的最新状态和反馈。归档会话仍计入。没有样本的比例或耗时显示「—」。页面访问量、Token 和费用尚未纳入。</p></footer>
        </> : null}
      </> : null}
    </div>
  </main>
