...
import { simcAttributeValue, simcDate, simcFightStyles, simcLabel, simcNumber, simcStatuses } from './simc-presentation'
...
export default function WebSimcReport({ job, onBack, onRefresh, refreshing }: Props) {
  ...
  return <article className={styles['report']} data-job-status={job.status}>
    ...
    <div className={styles['reportMeta']}>
      <span>{report?.engine.version ? `运行引擎 SimulationCraft ${report.engine.version}` : '运行版本未记录'}</span>
      {report?.engine.gameVersion ? <span>游戏 {report.engine.gameVersion}</span> : null}
      {report?.engine.build ? <span>Build {report.engine.build}</span> : null}
    </div>
    ...
    {job.status === 'failed' || job.status === 'cancelled' ? <section className={styles['empty']}>
      ...
      {job.errorCode ? <code className={styles['failureCode']}>{job.errorCode}</code> : null}
    </section> : null}
    ...
    {job.result ? <>
      ...
      {!report ? <div className={styles['notice']}>该历史任务仅保存了结果摘要，未记录技能、Buff、装备天赋及引擎版本详情。</div> : <>
        ...
        <div className={styles['reportColumns']}>
          <section className={styles['card']}><h3>Buff 覆盖</h3>{report.buffs.length ? <div className={styles['buffList']}>
            {report.buffs.map((buff, index) => <div key={`${buff.name}-${index}`}><div className={styles['summaryRow']}><span>{buff.name}</span><strong>{simcNumber(buff.uptime)}%</strong></div><div className={styles['barTrack']}><i style={{ width: `${Math.min(100, Math.max(0, buff.uptime))}%` }} /></div></div>)}
          </div> : <p className={styles['missing']}>本次报告未记录 Buff 覆盖。</p>}</section>
          <section className={styles['card']}><h3>资源</h3>{report.resources.length ? <div className={styles['tableScroll']}><table><thead><tr><th>资源</th><th>获得</th><th>消耗</th></tr></thead><tbody>
            {report.resources.map((resource, index) => <tr key={`${resource.name}-${index}`}><td>{simcLabel(resource.name)}</td><td>{simcNumber(resource.gained)}</td><td>{simcNumber(resource.lost)}</td></tr>)}
        ...
        <section className={styles['card']}><h3>装备与天赋</h3><p className={styles['muted']}>本次运行使用的角色资料</p>
          {report.gear.length ? <div className={styles['tableScroll']}><table><thead><tr><th>部位</th><th>物品 ID</th><th>物品等级</th></tr></thead><tbody>
    ...
    <section className={styles['card']}><h3>本次模拟配置</h3>{scenario ? <dl className={styles['scenarioGrid']}>
      <div><dt>战斗类型</dt><dd>{simcFightStyles[scenario.fightStyle as keyof typeof simcFightStyles] ?? scenario.fightStyle}</dd></div>
    ...
    <details className={styles['provenance']}><summary>运行记录与来源</summary><dl>
      <div><dt>任务 ID</dt><dd>{job.id}</dd></div><div><dt>编译器</dt><dd>{job.compilerRevision}</dd></div>
      <div><dt>运行身份</dt><dd>{job.runtimeRevision}</dd></div><div><dt>场景校验</dt><dd>{job.scenarioHash}</dd></div>
      {job.result ? <div><dt>Profile 校验</dt><dd>{job.result.profileSha256}</dd></div> : null}
    </dl></details>
  </article>
}
