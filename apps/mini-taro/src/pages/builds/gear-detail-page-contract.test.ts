...
describe('gear detail fixed workbench contract', () => {
  ...
  it('saves complete gear drafts, asks before importing, and clears every equipped slot on reset', () => {
    ...
    expect(pageSource).toContain("const source = await chooseActionSheetEntry(importSources, (entry) => entry.label)")
    expect(pageSource).toContain("const selected = await chooseActionSheetEntry(source.options, (entry) => entry.label)")
