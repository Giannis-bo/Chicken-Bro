...
describe('formal SimC client', () => {
  it('opts into the same zhCN report for Web sessions without putting ownership in the URL', async () => {
    ...
    expect(transport.requests.map(call => call.path)).toEqual(Array(2).fill('/api/v2/simc/jobs/job-one?view=workbench&scenarioVersion=3&reportLocale=zhCN'))
  })
  ...
  it('opts into bounded workbench context and preserves actual scenario options', async () => {
    ...
    expect(transport.requests[0]?.path).toBe('/api/v2/simc/jobs?view=workbench&scenarioVersion=3')
