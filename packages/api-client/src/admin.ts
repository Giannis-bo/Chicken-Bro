import { isAdminAccess, isAdminOverview, type AdminAccess, type AdminOverview } from '@wow-mini/domain'
...
export interface AdminClient {
  ...
  overview(auth: Auth, start: string, end: string): Promise<ApiResult<AdminOverview | null>>
}
export function createAdminClient(transport: ApiTransport): AdminClient {
  ...
  return {
    ...
    overview: (auth,start,end) => transport.request<AdminOverview | null>(apiV2Path(`/admin/overview?start=${encodeURIComponent(start)}&end=${encodeURIComponent(end)}`), { ...options(auth), validate: isAdminOverview }),
  }
}
