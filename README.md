...
最新只读快照显示根分区约余 8.58 GB，而 PostgreSQL 目录约 39.63 GB。控制台现有的唯一系统盘快照创建于 2026-03-17，早于当前数据库状态且没有恢复验证；广州地域也没有待挂载云硬盘，服务器没有独立挂载或已配置的对象存储备份客户端。创建干净 `chickenbro_prod` 前必须先完成新的独立恢复副本与恢复验证并精确清理无引用旧数据，或扩容。不能提前删除 `wow_test`、正式部署或唯一恢复点绕过容量门禁。
