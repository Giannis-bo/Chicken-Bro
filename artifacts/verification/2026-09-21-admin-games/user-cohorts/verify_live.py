...
with connect() as conn:
 ...
 for game in ('wow','poe2'):
  ...
  first_chats=conn.execute('''SELECT r.user_id,min(r.started_at) FROM chat.agent_runs r
   JOIN chat.conversations c ON c.id=r.conversation_id AND c.user_id=r.user_id
   WHERE c.game=%s AND r.user_id=ANY(%s::uuid[]) AND r.started_at < %s GROUP BY r.user_id''',(game,owners,window.end)).fetchall()
  assert data['users']=={'total':len(first_chats),'new':sum(at>=window.start for _,at in first_chats),'active':len({r[0] for r in runs})}
  assert sum(day['newUsers'] for day in data['daily'])==data['users']['new']
