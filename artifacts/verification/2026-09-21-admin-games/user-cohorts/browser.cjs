...
const output=path.join(root,'artifacts/verification/2026-09-21-admin-games/user-cohorts')
...
try{
 ...
 for(const width of [1440,390,320]){
  ...
  assert.equal(await page.getByText('当前游戏 · 期间首次提问的用户',{exact:true}).count(),1)
  await page.getByRole('button',{name:'POE2',exact:true}).click()
