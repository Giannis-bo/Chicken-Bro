...
SRC=Path('/opt/chickenbro-candidates/admin-games-20260921')
PACK=Path('/var/lib/chickenbro/releases/admin-games-20260921-final')
TARGET=Path('/opt/chickenbro-releases/admin-games-20260921-final')
WEB=Path('/var/www/chickenbro-web/releases/admin-games-20260921-final')
LINK=Path('/opt/chickenbro');WLINK=Path('/var/www/chickenbro-web/current')
UNITS=('chickenbro-api','chickenbro-worker')
FILES=('server/app/admin/application.py','server/app/admin/repository.py','server/app/api/routes/admin.py')
...
with open('/run/lock/chickenbro-release.lock','a') as lock:
 ...
 if mode=='prepare':
  ...
  expected=json.loads((SRC/'artifacts/verification/2026-09-21-admin-games/source-sha256.json').read_text())['files']
