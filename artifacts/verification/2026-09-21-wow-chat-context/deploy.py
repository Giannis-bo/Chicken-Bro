"""Cloud-only exact-file WoW Chat repair with retained application recovery."""
import fcntl,hashlib,json,os,shutil,subprocess,sys,time
from pathlib import Path
from urllib.request import urlopen
SRC=Path('/opt/chickenbro-candidates/wow-chat-context-20260921')
PACK=Path('/var/lib/chickenbro/releases/wow-chat-context-20260921')
TARGET=Path('/opt/chickenbro-releases/wow-chat-context-20260921')
WEB=Path('/var/www/chickenbro-web/releases/wow-chat-context-20260921')
LINK=Path('/opt/chickenbro');WLINK=Path('/var/www/chickenbro-web/current')
UNITS=('chickenbro-api','chickenbro-worker')
FILES=('server/app/chickenbro/codex_adapter.py',)
...
with open('/run/lock/chickenbro-release.lock','a') as lock:
 ...
 if mode=='prepare':
  ...
  expected=json.loads((SRC/'artifacts/verification/2026-09-21-wow-chat-context/source-sha256.json').read_text())['files']
