"""Exact admin release. Initial promotion denies all admin access pending real-session binding."""
import argparse
import hashlib
import json
import os
...
target = Path('/opt/chickenbro-releases/admin-ops-' + commit)
web = Path('/var/www/chickenbro-web/releases/admin-ops-' + commit)
payload = manifest_path.parent
env_file = Path('/etc/chickenbro-admin-ops-' + commit[:12] + '.env')
drops = [Path('/etc/systemd/system/'+unit+'.service.d/99-zzzzzz-admin-ops-'+commit[:12]+'.conf') for unit in ('chickenbro-api','chickenbro-worker')]
...
def switch(link, destination):
    temporary = link.with_name('.admin-next-' + uuid4().hex)
...
for unit in ('chickenbro-api', 'chickenbro-worker'):
    ...
    assert all(current_env.get(k) == v for k,v in prior_env.items() if k.startswith(('WOW_', 'CHICKENBRO_', 'CODEX_')) and k != 'WOW_ADMIN_USER_ID'), 'effective environment drift'
    assert not current_env.get('WOW_ADMIN_USER_ID')
    pid = subprocess.check_output(['systemctl','show',unit,'-p','MainPID','--value'],text=True).strip()
