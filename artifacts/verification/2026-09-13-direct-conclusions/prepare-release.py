...
source = Path('/opt/chickenbro-candidates/direct-conclusions-20260913')
files = ['server/chickenbro_native_mcp.py', 'server/app/chickenbro/agent_skills.py', 'server/app/chickenbro/codex_adapter.py',
         'server/app/chickenbro/agent/AGENTS.md', *[
             'server/app/chickenbro/agent/skills/' + name + '.md'
             for name in ('mechanics', 'rankings', 'simc-experiment', 'wcl-analysis')]]
p = root / 'deploy.py'
s = p.read_text()
...
assert base.name == 'badcase-2294cb95095a3822e1f4e2c43e225bf0bfdb2f63'
