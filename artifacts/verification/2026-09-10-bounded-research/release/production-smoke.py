...
p=argparse.ArgumentParser();p.add_argument('mode',choices=['production']);p.add_argument('source');a=p.parse_args()
...
if a.mode=='candidate':
    ...
    private=Path('/var/lib/chickenbro-bounded-candidate-20260910');private.mkdir(mode=0o700,exist_ok=True)
...
if a.mode=='candidate':
    ...
    for kind,command in [('worker',['-m','server.app.worker.main','--worker-id','badcase-candidate']),('api',['-m','uvicorn','server.app.main:app','--host','127.0.0.1','--port','18790','--log-level','warning'])]:
        log=open('/var/lib/chickenbro-bounded-candidate-20260910/'+kind+'.log','ab')
...
with httpx.Client(base_url=base,timeout=510) as client:
    ...
    packets=json.loads((Path(__file__).parent/'production-cases.json').read_text())
    report['cases']=[]
    ...
    for packet in packets:
        ...
        (Path(__file__).parent/'production-progress.json').write_text(json.dumps(report,ensure_ascii=False,indent=2))
...
(Path(__file__).parent/'production.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n')
