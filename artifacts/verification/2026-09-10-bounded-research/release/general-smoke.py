...
if a.mode=='candidate':
    ...
    private=Path('/var/lib/chickenbro-bounded-release-candidate');private.mkdir(mode=0o700,exist_ok=True)
...
if a.mode=='candidate':
    ...
    for kind,command in [('worker',['-m','server.app.worker.main','--worker-id','badcase-general-candidate']),('api',['-m','uvicorn','server.app.main:app','--host','127.0.0.1','--port','18790','--log-level','warning'])]:
        log=open('/var/lib/chickenbro-bounded-release-candidate/'+kind+'.log','ab')
