...
with open('/run/lock/chickenbro-candidate-validation.lock', 'a') as lock:
    ...
    assert not (ROOT / 'started.json').exists(), 'Existing experiment must be inspected, not rerun'
    ...
    write('started.json', {'at': time.time(), 'binarySha256': EXPECTED_BINARY, 'baseProfileSha256': EXPECTED_PROFILE, 'priorInspectionInvocations': 1, 'maxNewInvocations': 3})
    query = subprocess.run([str(ENGINE), OVERRIDE, 'spell_query=spell.id=1250508'], capture_output=True, text=True, timeout=20)
    (ROOT / 'zero-penalty-spell-query.txt').write_text(query.stdout + query.stderr)
    assert query.returncode == 0
    effect2 = query.stdout.split('#2 (id=1254615)', 1)[1]
    assert 'Scaled Value: 0' in effect2 and 'coefficient=0' in effect2
