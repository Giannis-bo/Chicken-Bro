...
ALLOWED = frozenset(('server/app/simulation/wcl_talents.py', 'server/app/simulation/data/trait-labels-12.1.0.json', 'server/app/simulation/data/item-variants-12.1.0.json', 'server/app/simulation/data/trait-paths-12.1.0.json', 'server/app/simulation/data/traits-12.1.0.json', 'server/app/simulation/data/localization/12.1.0.69814/catalog.json.gz', 'server/app/simulation/data/localization/12.1.0.69814/engine-rules.json', 'server/app/simulation/data/localization/12.1.0.69814/manifest.json', 'server/chickenbro_simc_runtime_update.sh'))
...
class Release:
    ...
    def engine_verify(self, commit):
        release = Path('/opt/wow-simc/releases') / commit
        require(release.is_dir() and not release.is_symlink(), 'invalid engine release')
        require((release / '.commit').read_text().strip() == commit, 'engine commit mismatch')
        require(sha(release / 'simc') == (release / 'binary.sha256').read_text().strip(), 'engine binary mismatch')
        expected = json.loads((self.path.parent / 'engine-manifest.json').read_text())
        require(sha(release / 'simc') == expected[commit]['binarySha256'], 'engine manifest mismatch')
        return release

    def engine_switch(self, commit):
        old = 'f50a2121bf894570146507496f3e113bff68e445'
        new = 'ac0f3a3c7ff9e521137c0ca1760d548330c697f3'
        link = Path('/opt/wow-simc/current')
        require(link.resolve().name in (old, new), 'foreign engine pointer')
        self.engine_verify(link.resolve().name)
        release = self.engine_verify(commit)
        switch(link, release)
        marker = Path('/opt/wow-simc/.commit-next-' + uuid4().hex)
        marker.write_text(commit + '\n'); marker.chmod(0o644)
        os.replace(marker, Path('/opt/wow-simc/.commit'))

    def verify(self, current):
        commit = ('ac0f3a3c7ff9e521137c0ca1760d548330c697f3' if current == self.target
                  else 'f50a2121bf894570146507496f3e113bff68e445')
        require(Path('/opt/wow-simc/current').resolve() == self.engine_verify(commit), 'engine pointer mismatch')
    ...
    def activate(self, current, destination):
        with self.idle_fence() as conn:
            ...
            self.engine_switch('ac0f3a3c7ff9e521137c0ca1760d548330c697f3' if destination == self.target else 'f50a2121bf894570146507496f3e113bff68e445')
        self.start()
...
def main():
    ...
    with open('/run/lock/chickenbro-release.lock', 'a') as lock:
        ...
        with open('/run/lock/chickenbro-simc-runtime-update.lock', 'a') as engine_lock:
