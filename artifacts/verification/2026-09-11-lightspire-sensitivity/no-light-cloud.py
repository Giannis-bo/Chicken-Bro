"""One registered no-extra-mastery endpoint; preserve the completed three-point results."""
...
with open('/run/lock/chickenbro-candidate-validation.lock','a') as lock:
 ...
 assert json.loads((ROOT/'result.json').read_text())['finished']
 assert not (ROOT/'no-light-mastery-started.json').exists(),'Never repeat attempted endpoint'
 ...
 write('no-light-endpoint-registration.json',{'at':time.time(),'sourceProfileSha256':PROFILE,'binarySha256':BINARY,'newCombatCeiling':1,'priorCombatRuns':3,'priorSpellQueries':2,'reason':'Literal multiplier0 is not a no-benefit control; directly remove only effect1275873 rating magnitude.'})
 results={'binarySha256':BINARY,'sourceProfileSha256':PROFILE,'cases':[]}
 for val in ['no-light-mastery']:
  label=str(val);profile='override.spell_data=effect.1275873.coefficient=0\n'+baseline+'midnight.lightspire_core_duration_multiplier=0.5\n'
  ...
  write('no-light-result.json',results)
  print(json.dumps({'multiplier':val,'dps':metric['mean'],'uptime':buff['uptime']}),flush=True)
 assert sha(ENGINE.read_bytes())==BINARY
 results['finished']=True;results['combatRuns']=1;write('no-light-result.json',results)
