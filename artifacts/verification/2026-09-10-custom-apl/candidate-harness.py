...
p=argparse.ArgumentParser();p.add_argument('mode',choices=['candidate']);p.add_argument('source');a=p.parse_args()
...
if a.mode=='candidate':
    ...
    assert exists=='1', 'Existing isolated database required; no provisioning authorized'
    private=Path('/var/lib/chickenbro-apl-candidate-20260910');private.mkdir(mode=0o700,exist_ok=True)
    ...
    with psycopg.connect(env['WOW_DATABASE_URL']) as c:
        assert c.execute("SELECT count(*) FROM chat.agent_runs WHERE status='streaming'").fetchone()[0]==0, 'Candidate busy'
        assert c.execute("SELECT count(*) FROM ops.job_queue WHERE status IN ('queued','running')").fetchone()[0]==0, 'Candidate queue busy'
...
def cleanup():
    ...
    if children:
        with psycopg.connect(env['WOW_DATABASE_URL']) as c:
            active=c.execute("SELECT count(*) FROM chat.agent_runs WHERE status='streaming'").fetchone()[0]
        if active:
            print(json.dumps({'cleanup':'left_candidate_running_for_active_generation'}),flush=True);return
...
with psycopg.connect(env['WOW_DATABASE_URL']) as c:
    ...
    for uid in owners:
        ...
        c.execute("INSERT INTO identity.auth_sessions(token_hash,user_id,kind,expires_at) VALUES (%s,%s,'web_cookie',now()+interval '40 minutes')",(hashlib.sha256(t.encode()).hexdigest(),uid))
if a.mode=='candidate':
    u=pwd.getpwnam('ubuntu')
    for kind,command in [('worker',['-m','server.app.worker.main','--worker-id','badcase-candidate']),('api',['-m','uvicorn','server.app.main:app','--host','127.0.0.1','--port','18890','--log-level','warning'])]:
        log=open('/var/lib/chickenbro-apl-candidate-20260910/'+kind+'.log','ab')
...
with httpx.Client(base_url=base,timeout=510) as client:
    ...
    packets=json.loads((Path(__file__).parent/'cases.json').read_text())
    report['cases']=[]
    suite_started=time.monotonic()
    source_calls=0
    for packet in packets:
        assert time.monotonic()-suite_started < 1500, 'suite time budget exhausted'
        assert source_calls < 40, 'source call budget exhausted'
        ...
        with psycopg.connect(env['WOW_DATABASE_URL']) as c:
            entry['tool_results']=[{'operation':x[0],'state':x[1],'result':x[2]} for x in c.execute('SELECT operation,state,result_json FROM chat.tool_results WHERE run_id=%s ORDER BY started_at,call_id',(first['runId'],)).fetchall()]
        source_calls += len(entry['tool_results'])
        report['cases'].append(entry)
        (Path(__file__).parent/'candidate-progress.json').write_text(json.dumps(report,ensure_ascii=False,indent=2))
        print(json.dumps({'case':entry['case'],'seconds':entry['seconds'],'toolCount':len(entry['tool_results']),'answer':entry['answer']},ensure_ascii=False),flush=True)
cleanup();tokens.clear();report.update(passed=True,sessionsRevoked=True)
(Path(__file__).parent/'candidate.json').write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n')
print(json.dumps({'completed':len(report['cases']),'sessionsRevoked':True},ensure_ascii=False),flush=True)
