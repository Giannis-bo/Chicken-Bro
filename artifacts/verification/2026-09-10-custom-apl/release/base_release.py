...
ALLOWED = frozenset(('server/app/api/routes/simc.py', 'server/app/chickenbro/agent/AGENTS.md', 'server/app/chickenbro/simulation_tools.py', 'server/app/simulation/action_lists.py', 'server/app/simulation/compiler.py', 'server/app/simulation/experiments.py', 'server/app/simulation/readiness.py', 'server/app/simulation/worker.py', 'server/chickenbro_native_mcp.py'))
...
class Release:
    def __init__(self, manifest, connect=None):
        ...
        require(set(m) <= {'sourceCommit', 'expectedBackend', 'expectedWeb', 'baseInventory',
                'files', 'baseHashes', 'webFiles', 'environmentHashes', 'newWebFiles'}, 'unknown manifest fields')
        ...
        self.target = Path('/opt/chickenbro-releases/custom-apl-' + m['sourceCommit'])
