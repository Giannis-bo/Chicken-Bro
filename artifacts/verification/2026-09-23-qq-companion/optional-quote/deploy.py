...
ROOT=Path('/opt/chickenbro-qq-releases/20260923-companion-quote-v5')
OLD=Path('/opt/chickenbro-qq-releases/20260923-companion-active-v4')
CONFIG=Path('/etc/chickenbro-qq-channel/channel.json')
ENV=Path('/etc/chickenbro-qq-channel/production.env')
EVIDENCE=Path('/var/lib/chickenbro/qq-companion-quote-release.json')
 ...
 for _ in range(40):
  if output('systemctl','is-active','chickenbro-qq-channel')=='active':
   ...
   if p.exists():
    ...
    if h.get('qqOnline') and h.get('runtimeRevision')=='qq-companion-20260923-quote-v5' and time.time()-h['observedAt']<8:return h
  time.sleep(.5)
...
with open('/run/lock/chickenbro-release.lock','a') as lock:
 ...
 raw=json.loads(CONFIG.read_text());assert raw['mode']=='companion' and raw['proactiveEnabled'] is True
 ...
 state={'oldRelease':str(OLD),'newRelease':str(ROOT),'backup':str(backup),'websiteBefore':before,'manifestFiles':len(m['files']),'userAcceptedScope':'mention_and_proactive_text_and_expression_visible','proactiveGroupAcceptance':'user_confirmed_20260923'}
 ...
 try:
  ...
  # Additive output metadata; historical replies retain quote=true.
  run('sudo','-u','postgres','/opt/chickenbro-runtime/bin/python','-c','import importlib,psycopg;from pathlib import Path;c=psycopg.connect("postgresql:///chickenbro_qq_channel",autocommit=True);importlib.import_module("server.migrations.product.apply").apply_product_migrations(c,Path("server/migrations/product"))',cwd=ROOT)
  state['migration']='0020_qq_optional_quote.sql'
  env=[x for x in ENV.read_text().splitlines() if not x.startswith('WOW_CODEX_RUNTIME_REVISION=')]
  env.append('WOW_CODEX_RUNTIME_REVISION=qq-companion-20260923-quote-v5');atomic(ENV,'\n'.join(env)+'\n')
  point(ROOT);switched=True
  run('systemctl','start','chickenbro-qq-worker','chickenbro-qq-channel');state['channelOnline']=healthy()['qqOnline']
  ...
  print(json.dumps({k:state[k] for k in ('status','newRelease','manifestFiles','restoredCountsMatch','channelOnline','proactiveEnabled','websiteReady')},ensure_ascii=False))
