...
path=Path('/var/lib/chickenbro/images-release-'+mode+'-private.json')
path.write_text(json.dumps({'token':token,'csrf':csrf,'otherToken':other_token,'owner':str(owner),'other':str(other),'conversationId':conversation['id'],'imageId':image['id'],'dataUrl':data}));path.chmod(0o600)
