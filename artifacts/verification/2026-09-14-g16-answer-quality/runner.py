...
def mcp(args):
    ...
    from g16_fixtures import FixtureSession
...
def run_case(args):
    ...
    from g16_fixtures import CASES
    ...
    history=list(case.get('history',[]));turns=[]
    for index,prompt in enumerate(case['turns']):
        ...
        try:
            for event in runtime.stream(prompt=json.dumps({'messages':history,'benchmarkScope':'Fixed retained source paraphrases and labeled synthetic variants; no live game observation or engine execution.'},ensure_ascii=False),timeout_seconds=args.timeout,tool_context=SimpleNamespace(run_id=run_id)):
        ...
        output.update(source=str(source),coreSha256=hashlib.sha256((source/'server/app/chickenbro/agent/AGENTS.md').read_bytes()).hexdigest(),fixtureSha256=hashlib.sha256(Path(__file__).with_name('g16_fixtures.py').read_bytes()).hexdigest(),rubric=case['rubric'],requiredEvidence=case.get('requiredEvidence',[]),configuration={k:profile_original(os.environ['WOW_CODEX_PROFILE']).get(k) for k in ('model','model_reasoning_effort')})
