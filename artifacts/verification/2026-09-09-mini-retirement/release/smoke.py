...
if a.mode=='candidate':
    ...
    dsn=urlsplit(env['WOW_DATABASE_URL']);db='chickenbro_mini_retire_candidate_20260909'
    ...
    private=Path('/var/lib/chickenbro-mini-retirement-candidate');private.mkdir(mode=0o700,exist_ok=True)
    ...
    env.update(WOW_DATABASE_URL=urlunsplit(dsn._replace(path='/'+db)),WOW_APP_ENV='test',WOW_API_V2_PORT='18790',
        ...
        WOW_CODEX_JOBS_DIR=str(private/'jobs'),WOW_WEB_COOKIE_NAME='__Host-mini-retirement-session',WOW_WEB_CSRF_COOKIE_NAME='__Host-mini-retirement-csrf',
...
with psycopg.connect(env['WOW_DATABASE_URL']) as c:
    if a.mode=='candidate':
        ...
        for uid in owners:
            ...
            c.execute("INSERT INTO identity.user_identities(id,user_id,provider,app_context,provider_subject) VALUES (%s,%s,'qq',%s,%s)",(uuid4(),uid,env['WOW_QQ_APPID'],'mini-retirement-isolated-'+uuid4().hex))
    else:
...
if a.mode=='candidate':
    ...
    for kind,command in [('worker',['-m','server.app.worker.main','--worker-id','mini-retirement-candidate']),('api',['-m','uvicorn','server.app.main:app','--host','127.0.0.1','--port','18790','--log-level','warning'])]:
        log=open('/var/lib/chickenbro-mini-retirement-candidate/'+kind+'.log','ab')
...
with httpx.Client(base_url=base,timeout=510) as client:
    ...
    conv=req('POST','/chat/conversations',201,json={'title':'Web 退役发布验证（复用测试账号）'},headers={**headers,'Idempotency-Key':uuid4().hex}).json()
