...
for unit in ['chickenbro-api','chickenbro-worker']:
    ...
    services[unit]={'pid':int(pid),'cwd':str(base),'durable':True,'simcSpecs':'all'}
...
with psycopg.connect(env['WOW_DATABASE_URL']) as c:
    ...
    assert providers.get('wechat_mini',0)==0 and providers.get('qq')==18
...
backup=Path('/var/backups/chickenbro-mini-retirement/20260909_retire186_02/before.dump')
assert hashlib.sha256(backup.read_bytes()).hexdigest()=='387c760fa3aca28d3683df850b43774ad48bfc9c284b98ad39f6934d6e7a3ce6'
print(json.dumps({'sourceCommit':m['sourceCommit'],'backendInventoryMatched':True,'removedBackendFilesAbsent':True,'publicWebFilesMatched':len(m['webFiles']),'oldBackendAndWebUnchanged':True,'dataBackupHashMatched':True,'services':services,'readiness':readiness,'providerCounts':providers,'resultProtectionTriggers':triggers,'invalidConstraints':invalid},indent=2))
