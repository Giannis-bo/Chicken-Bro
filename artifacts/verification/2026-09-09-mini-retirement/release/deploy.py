"""Exact Mini retirement release. Preserves runtime config, data and recovery pointers."""
import argparse
import hashlib
import json
import os
...
target = Path('/opt/chickenbro-releases/mini-retirement-' + commit)
web = Path('/var/www/chickenbro-web/releases/mini-retirement-' + commit)
payload = manifest_path.parent
env_file = Path('/etc/chickenbro-mini-retirement-' + commit[:12] + '.env')
drops = []
...
for unit in ('chickenbro-api', 'chickenbro-worker'):
    ...
    assert all(current_env.get(k) == v for k,v in prior_env.items() if k.startswith(('WOW_', 'CHICKENBRO_', 'CODEX_'))), 'effective environment drift'
    pid = subprocess.check_output(['systemctl','show',unit,'-p','MainPID','--value'],text=True).strip()
