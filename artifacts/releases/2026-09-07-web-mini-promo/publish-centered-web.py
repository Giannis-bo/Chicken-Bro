...
base=Path('/tmp/chickenbro-brand-centered-8a047efda')
...
assert m['commit'].startswith('8a047efda') and len(m['commit'])==40
root=Path('/var/www/chickenbro-web/releases')/('brand-centered-'+m['commit'])
current=Path('/var/www/chickenbro-web/current')
old=Path('/opt/chickenbro-releases/95fedba9c5fbc8088a652a887b70dad144164313/web')
assert current.is_symlink() and current.resolve()==old
assert not root.exists() and not root.is_symlink()
nextlink=current.with_name('current.brand-centered-next')
...
try:
    ...
    with urllib.request.urlopen('https://www.chickenbro.cloud/css/app.css',timeout=15) as r: actual=hashlib.sha256(r.read()).hexdigest()
    assert actual==m['files']['css/app.css']
except Exception:
    ...
    if nextlink.is_symlink():nextlink.unlink()
print(json.dumps({'status':'published','webCommit':m['commit'],'webRoot':str(root),'rollbackWeb':str(old),'cssHashVerified':True,'apiWorkerMiniCommit':'95fedba9c5fbc8088a652a887b70dad144164313'}))
