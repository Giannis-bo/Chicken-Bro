...
DROPINS = tuple(Path('/etc/systemd/system') / (u + '.d') / '99-chat-progress-release-20260907.conf' for u in UNITS)
...
def switch(link, target):
    pending = link.with_name(link.name + '.chat-progress-next')
...
def promote(manifest, root, apply):
    ...
    if any(p.with_name(p.name + '.chat-progress-next').exists() or p.with_name(p.name + '.chat-progress-next').is_symlink() for p in (CODE, WEB)):
        raise ValueError('unfinished link switch')
